...
crypto_Des56EcbEncryptor_TestClass::crypto_Des56EcbEncryptor_TestClass()
    : mTestTag("tc::crypto::Des56EcbEncryptor"), mTestResults()
{
}

void crypto_Des56EcbEncryptor_TestClass::runAllTests(void)
{
    test_Constants();
    test_UseClassEnc();
    test_UseClassDec();
...
const std::string &crypto_Des56EcbEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Des56EcbEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
...
void crypto_Des56EcbEncryptor_TestClass::test_Constants()
{
    ...
    try
    {
        ...
        if (tc::crypto::Des56EcbEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Des56EcbEncryptor::kBlockSize,
                kExpectedBlockSize));
        }

        // check key size
        static const size_t kExpectedKeySize = 8;
        if (tc::crypto::Des56EcbEncryptor::kKeySize != kExpectedKeySize)
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Des56EcbEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Des56EcbEncryptor_TestClass::test_UseClassEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        TdesDesEcbEncryptorUtil::generateDesEcbTestVectors_Nist(tests, 56);
        if (tests.begin() == tests.end())
        {
            throw tc::TestException("No test vectors");
        }

        tc::crypto::Des56EcbEncryptor cryptor;
...
void crypto_Des56EcbEncryptor_TestClass::test_UseClassDec()
{
    TestResult test_result;
...
void crypto_Des56EcbEncryptor_TestClass::test_UseUtilFuncEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            for (size_t itr = 0; itr < test->iteration_count; itr++)
            {
                tc::crypto::EncryptDes56Ecb(data.data(), data.data(), data.size(), test->key.data(), test->key.size());
            }
...
void crypto_Des56EcbEncryptor_TestClass::test_UseUtilFuncDec()
{
    TestResult test_result;
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            for (size_t itr = 0; itr < test->iteration_count; itr++)
            {
                tc::crypto::DecryptDes56Ecb(data.data(), data.data(), data.size(), test->key.data(), test->key.size());
            }
...
void crypto_Des56EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Des56EcbEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Des56EcbEncryptor::kBlockSize);
...
void crypto_Des56EcbEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        try
        {
            cryptor.initialize(tests[0].key.data(), tc::crypto::Des56EcbEncryptor::kKeySize - 1);
...
void crypto_Des56EcbEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        try
        {
            cryptor.encrypt(data.data(), tests[0].base_data.data(), tc::crypto::Des56EcbEncryptor::kBlockSize - 1);
...
void crypto_Des56EcbEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].base_data.data(), tc::crypto::Des56EcbEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
