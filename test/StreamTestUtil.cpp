...
void StreamTestUtil::constructor_TestHelper(
    tc::io::IStream &stream,
    int64_t stream_length,
    int64_t exp_pos_res,
    bool exp_canread_res,
    bool exp_canwrite_res,
    bool exp_canseek_res)
{
    ...
    if (length_res != stream_length)
    {
        throw tc::TestException(
            fmt::format("Stream did not have length {:d} (actual {:d})", stream_length, length_res));
    }
    ...
    {
        throw tc::TestException(
            fmt::format("Stream property canRead() was not {} (actual {})", exp_canread_res, can_read));
    }

    if (can_write != exp_canwrite_res)
    {
        throw tc::TestException(
            fmt::format("Stream property canWrite() was not {} (actual {})", exp_canwrite_res, can_write));
    }

    if (can_seek != exp_canseek_res)
    {
        throw tc::TestException(
            fmt::format("Stream property canSeek() was not {} (actual {})", exp_canseek_res, can_seek));
    }
}

void StreamTestUtil::seek_TestHelper(
    tc::io::IStream &stream,
    int64_t seek_offset,
    tc::io::SeekOrigin seek_origin,
    ...
    int64_t exp_pos_res)
{
    ...
    {
        throw tc::TestException(
            fmt::format("Stream did not return position from seek {:d} (actual {:d})", exp_seek_res, seek_res));
    }
...
void StreamTestUtil::read_TestHelper(
    tc::io::IStream &stream,
    int64_t seek_offset,
    tc::io::SeekOrigin seek_origin,
    size_t dst_size,
    ...
    const byte_t *expected_data)
{
    ...
    {
        throw tc::TestException(
            fmt::format("Stream did not read expected number of bytes {:d} (actual {:d})", exp_read_res, read_res));
    }
    ...
    if (expected_data != nullptr && memcmp(data.data(), expected_data, exp_read_res) != 0)
    {
        throw tc::TestException(fmt::format(
            "Stream did not read expected bytes (read: \"{}\", expected: \"{}\"",
            tc::cli::FormatUtil::formatBytesAsString(data.data(), read_res, true, ""),
            tc::cli::FormatUtil::formatBytesAsString(expected_data, exp_read_res, true, "")));
    }
...
void StreamTestUtil::write_TestHelper(
    tc::io::IStream &stream,
    int64_t seek_offset,
    tc::io::SeekOrigin seek_origin,
    tc::ByteData &data,
    ...
    int64_t exp_length_res)
{
    ...
    {
        throw tc::TestException(
            fmt::format("Stream did not have length {:d} (actual {:d})", exp_length_res, length_res));
    }
}