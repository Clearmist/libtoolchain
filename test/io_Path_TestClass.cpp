...
io_Path_TestClass::io_Path_TestClass() : mTestTag("tc::io::Path"), mTestResults() {}
    ...
    try
    {
        ...
        if (path.size() != expected_elements.size())
        {
            throw tc::TestException(fmt::format(
                "InitializerList constructor did not create a path with {} elements (.size() == {})",
                expected_elements.size(), path.size()));
        }
        ...
        for (auto pathItr = path.begin(), expItr = expected_elements.begin(); pathItr != path.end();
             pathItr++, expItr++)
        {
            if (*pathItr != *expItr)
            {
                throw tc::TestException(fmt::format(
                    "InitializerList constructer created a path with unexpected elements (Path contained {}, expected "
                    "{})",
                    std::list<std::string>(path.begin(), path.end()), expected_elements));
            }
        }
...
void io_Path_TestClass::test_Constructor_u8string()
{
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            {
                throw tc::TestException(fmt::format(
                    "Failed to convert \"{}\" to path. (Path contained {}, expected {})", itr->path_string,
                    std::list<std::string>(path.begin(), path.end()),
                    std::list<std::string>(itr->expected_path.begin(), itr->expected_path.end())));
            }
        }
        ...
        try
        {
            ...
            throw tc::TestException("Path(const std::string&) constructor did not throw tc::ArgumentException where "
                                    "there was a mix of path separators.");
        }
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            if (path != itr->expected_path)
            {
                ...
                throw tc::TestException(fmt::format(
                    "Failed to convert u\"{}\" to path. (Path contained {}, expected {})", u8_string,
                    std::list<std::string>(path.begin(), path.end()),
                    std::list<std::string>(itr->expected_path.begin(), itr->expected_path.end())));
            }
        }
        ...
        try
        {
            ...
            throw tc::TestException("Path(const std::u16string&) constructor did not throw tc::ArgumentException where "
                                    "there was a mix of path separators.");
        }
        catch (const tc::ArgumentException &)
        {
...
void io_Path_TestClass::test_Constructor_u32string()
{
    ...
    try
    {
        ...
        {
            tc::io::Path(U"A/Mix\\Of/Path\\Separators");
            throw tc::TestException("Path(const std::u32string&) constructor did not throw tc::ArgumentException where "
                                    "there was a mix of path separators.");
        }
        catch (const tc::ArgumentException &)
        {
...
void io_Path_TestClass::test_Method_substr_withPosLen()
{
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            {
                throw tc::TestException(fmt::format(
                    ".subpath(pos={},len={}) returned incorrect sub path (returned {}, expected {})", itr->pos,
                    itr->len, std::list<std::string>(subpath.begin(), subpath.end()),
                    std::list<std::string>(itr->exp_subpath.begin(), itr->exp_subpath.end())));
            }
        }
...
void io_Path_TestClass::test_Method_substr_withIterator()
{
    ...
    try
    {
        ...
        for (auto testItr = tests.begin(); testItr != tests.end(); testItr++)
        {
            ...
            {
                throw tc::TestException(fmt::format(
                    ".subpath(begin,end) note({},{}) returned incorrect sub path (returned {}, expected {})",
                    testItr->pos, testItr->len, std::list<std::string>(subpath.begin(), subpath.end()),
                    std::list<std::string>(testItr->exp_subpath.begin(), testItr->exp_subpath.end())));
            }
        }
...
void io_Path_TestClass::test_Method_to_string()
{
    ...
    try
    {
        ...
        std::vector<Test> tests{
            ...
            {{"c:"}, "c:", "c:\\"},        // windows root path
            {{"data:"}, "data:", "data:"}, // this is a counter test to test the special case handling root paths in
                                           // POSIX and Win32 formatting
            {{"some", "relative", "path", "that", "has", "seven", "elements"},
             "some/relative/path/that/has/seven/elements",
             "some\\relative\\path\\that\\has\\seven\\elements"},              // relative path
            ...
            {{"C:", "Users", "TestUser", "Desktop", "hi.txt"},
             "C:/Users/TestUser/Desktop/hi.txt",
             "C:\\Users\\TestUser\\Desktop\\hi.txt"}, // absoulute Win32 path
            {{std::string(u8"示例文本"), std::string(u8"サンプルテキスト"), std::string(u8"δείγμα κειμένου"),
              std::string(u8"טקסט לדוגמה")},
             std::string(u8"示例文本/サンプルテキスト/δείγμα κειμένου/טקסט לדוגמה"),
             std::string(
                 u8"示例文本\\サンプルテキスト\\δείγμα κειμένου\\טקסט לדוגמה")} // non-ASCII characters (chinese,
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            {
                throw tc::TestException(fmt::format(
                    ".to_string(Format::Native) failed to format path as string. (returned: \"{}\", expected: \"{}\")",
                    native_str, exp_native_str));
            }

            if (posix_str != itr->exp_posix_str)
            {
                throw tc::TestException(fmt::format(
                    ".to_string(Format::POSIX) failed to format path as string. (returned: \"{}\", expected: \"{}\")",
                    posix_str, itr->exp_posix_str));
            }

            if (win32_str != itr->exp_win32_str)
            {
                throw tc::TestException(fmt::format(
                    ".to_string(Format::Win32) failed to format path as string. (returned: \"{}\", expected: \"{}\")",
                    win32_str, itr->exp_win32_str));
            }
        }
...
void io_Path_TestClass::test_Method_to_u16string()
{
    ...
    try
    {
        ...
        std::vector<Test> tests{
            ...
            {{"data:"}, u"data:", u"data:"}, // this is a counter test to test the special case handling root paths in
                                             // POSIX and Win32 formatting
            {{"some", "relative", "path", "that", "has", "seven", "elements"},
             u"some/relative/path/that/has/seven/elements",
             u"some\\relative\\path\\that\\has\\seven\\elements"},               // relative path
            ...
            {{"C:", "Users", "TestUser", "Desktop", "hi.txt"},
             u"C:/Users/TestUser/Desktop/hi.txt",
             u"C:\\Users\\TestUser\\Desktop\\hi.txt"}, // absoulute Win32 path
            {{std::string(u8"示例文本"), std::string(u8"サンプルテキスト"), std::string(u8"δείγμα κειμένου"),
              std::string(u8"טקסט לדוגמה")},
             std::u16string(u"示例文本/サンプルテキスト/δείγμα κειμένου/טקסט לדוגמה"),
             std::u16string(
                 u"示例文本\\サンプルテキスト\\δείγμα κειμένου\\טקסט לדוגמה")} // non-ASCII characters (chinese,
                                                                               // japanese, greek, hebrew)
        };
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            if (native_str != exp_native_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u16string(Format::Native) failed to format path as u16string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
            ...
            if (posix_str != itr->exp_posix_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u16string(Format::POSIX) failed to format path as u16string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
            ...
            if (win32_str != itr->exp_win32_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u16string(Format::Win32) failed to format path as u16string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
        }
...
void io_Path_TestClass::test_Method_to_u32string()
{
    ...
    try
    {
        ...
        std::vector<Test> tests{
            ...
            {{std::string(u8"示例文本"), std::string(u8"サンプルテキスト"), std::string(u8"δείγμα κειμένου"),
              std::string(u8"טקסט לדוגמה")},
             std::u32string(U"示例文本/サンプルテキスト/δείγμα κειμένου/טקסט לדוגמה"),
             std::u32string(
                 U"示例文本\\サンプルテキスト\\δείγμα κειμένου\\טקסט לדוגמה")} // non-ASCII characters (chinese,
                                                                               // japanese, greek, hebrew)
        };
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            if (native_str != exp_native_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u32string(Format::Native) failed to format path as u32string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
            ...
            if (posix_str != itr->exp_posix_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u32string(Format::POSIX) failed to format path as u32string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
            ...
            if (win32_str != itr->exp_win32_str)
            {
                ...
                throw tc::TestException(fmt::format(
                    ".to_u32string(Format::Win32) failed to format path as u32string. (returned: \"{}\", expected: "
                    "\"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
        }
...
void io_Path_TestClass::test_Operator_string()
{
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            {
                throw tc::TestException(fmt::format(
                    "operator string() failed to format path as string. (returned: \"{}\", expected: \"{}\")",
                    native_str, exp_native_str));
            }
        }
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            if (native_str != exp_native_str)
            {
                ...
                tc::string::TranscodeUtil::UTF16ToUTF8(exp_native_str, u8_exp_path_str);
                throw tc::TestException(fmt::format(
                    "operator u16string() failed to format path as u16string. (returned: \"{}\", expected: \"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
        }
    ...
    try
    {
        ...
        for (auto itr = tests.begin(); itr != tests.end(); itr++)
        {
            ...
            if (native_str != exp_native_str)
            {
                ...
                tc::string::TranscodeUtil::UTF32ToUTF8(exp_native_str, u8_exp_path_str);
                throw tc::TestException(fmt::format(
                    "operator u32string() failed to format path as u32string. (returned: \"{}\", expected: \"{}\")",
                    u8_path_str, u8_exp_path_str));
            }
        }
    ...
    try
    {
        ...
        if ((path_a < path_b) != true)
        {
            throw tc::TestException(
                "operator<() returned false for Path objects where (" + raw_path_a + ") < (" + raw_path_b + ")");
        }
        ...
        {
            throw tc::TestException(
                "operator<() returned true for Path objects where (" + raw_path_a + ") < (" + raw_path_c + ")");
        }
        ...
        {
            throw tc::TestException(
                "operator<() returned false for Path objects where (" + raw_path_a + ") < (" + raw_path_d + ")");
        }
