...
crypto_Aes192CbcEncryptor_TestClass::crypto_Aes192CbcEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes192CbcEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes192CbcEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes192CbcEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes192CbcEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes192CbcEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes192CbcEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes192CbcEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesCbcEncryptorUtil::generateAesCbcTestVectors_Nist(tests, 192);
...
void crypto_Aes192CbcEncryptor_TestClass::test_UseUtilFuncEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            tc::crypto::EncryptAes192Cbc(
                data.data(), test->plaintext.data(), data.size(), test->key.data(), test->key.size(), test->iv.data(),
                test->iv.size());
...
void crypto_Aes192CbcEncryptor_TestClass::test_UseUtilFuncDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            tc::crypto::DecryptAes192Cbc(
                data.data(), test->ciphertext.data(), data.size(), test->key.data(), test->key.size(), test->iv.data(),
                test->iv.size());
...
void crypto_Aes192CbcEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes192CbcEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes192CbcEncryptor::kBlockSize);
...
void crypto_Aes192CbcEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tc::crypto::Aes192CbcEncryptor::kKeySize - 1, tests[0].iv.data(),
                tests[0].iv.size());
        ...
        try
        {
            cryptor.initialize(
                ...
                tc::crypto::Aes192CbcEncryptor::kBlockSize - 1);
...
void crypto_Aes192CbcEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].plaintext.data(), tc::crypto::Aes192CbcEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
