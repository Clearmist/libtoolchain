...
io_MemoryStream_TestClass::io_MemoryStream_TestClass() : mTestTag("tc::io::MemoryStream"), mTestResults() {}
    ...
    try
    {
        ...
        if (memcmp(output_data.data(), data.data(), length) != 0)
        {
            throw tc::TestException(
                "Data in memory stream was not correct after being constructed from a ByteData object");
        }
...
void io_MemoryStream_TestClass::testSeekBeginNegative()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                ".seek() with position that was negative did not throw tc::ArgumentOutOfRangeException");
        }
        ...
        {
            throw tc::TestException(
                fmt::format(".seek() with position that was negative threw the wrong exception ({:s})", e.what()));
        }
...
void io_MemoryStream_TestClass::testSeekCurrentPastEnd()
{
    ...
    try
    {
        ...
        StreamTestUtil::seek_TestHelper(
            stream, length, tc::io::SeekOrigin::Current, seek_pos + length, seek_pos + length);
...
void io_MemoryStream_TestClass::testSeekEndPastEnd()
{
    ...
    try
    {
        ...
        StreamTestUtil::seek_TestHelper(
            stream, seek_pos, tc::io::SeekOrigin::End, length + seek_pos, length + seek_pos);
...
void io_MemoryStream_TestClass::testSeekEndTooNegative()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(".seek(SeekOrigin::End) with negative offset greater in magnitude than the stream "
                                    "length did not throw tc::ArgumentOutOfRangeException");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
        catch (const tc::Exception &e)
        {
            throw tc::TestException(fmt::format(
                ".seek(SeekOrigin::End) with negative offset greater in magnitude than the stream length threw the "
                "wrong exception ({:s})",
                e.what()));
        }
...
void io_MemoryStream_TestClass::testReadAllDataAvailable()
{
    ...
    try
    {
        ...
        StreamTestUtil::read_TestHelper(
            stream, stream_offset, tc::io::SeekOrigin::Begin, read_len, read_len, read_len, stream_length);
...
void io_MemoryStream_TestClass::testReadRequestsSubsetOfAvailableData()
{
    ...
    try
    {
        ...
        StreamTestUtil::read_TestHelper(
            stream, stream_offset, tc::io::SeekOrigin::Begin, read_len, read_len, read_len, int64_t(read_len));
...
void io_MemoryStream_TestClass::testReadSomeDataAvailable()
{
    ...
    try
    {
        ...
        StreamTestUtil::read_TestHelper(
            stream, stream_offset, tc::io::SeekOrigin::Begin, read_len, read_len, read_len - size_t(stream_offset),
            stream_length);
...
void io_MemoryStream_TestClass::testWriteAllDataWritable()
{
    ...
    try
    {
        ...
        StreamTestUtil::write_TestHelper(
            stream, stream_offset, tc::io::SeekOrigin::Begin, data, stream_expected_position);
...
void io_MemoryStream_TestClass::testWriteSomeDataWritable()
{
    ...
    try
    {
        ...
        StreamTestUtil::write_TestHelper(
            stream, stream_offset, tc::io::SeekOrigin::Begin, data, stream_expected_position, stream_expected_position);
