...
crypto_Aes256Encryptor_TestClass::crypto_Aes256Encryptor_TestClass()
    : mTestTag("tc::crypto::Aes256Encryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes256Encryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes256Encryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes256Encryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes256Encryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes256Encryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes256Encryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesEncryptorUtil::generateAesTestVectors_Nist(tests, 256);
...
void crypto_Aes256Encryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes256Encryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes256Encryptor::kBlockSize);
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes256Encryptor::kKeySize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Failed to throw correct exception where key_size==tc::crypto::Aes256Encryptor::kKeySize-1");
        }
