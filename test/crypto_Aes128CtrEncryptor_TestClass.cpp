...
crypto_Aes128CtrEncryptor_TestClass::crypto_Aes128CtrEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes128CtrEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes128CtrEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes128CtrEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes128CtrEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128CtrEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128CtrEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes128CtrEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesCtrEncryptorUtil::generateAesCtrTestVectors_Nist(tests, 128);
...
void crypto_Aes128CtrEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes128CtrEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes128CtrEncryptor::kBlockSize);
...
void crypto_Aes128CtrEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tc::crypto::Aes128CtrEncryptor::kKeySize - 1, tests[0].iv.data(),
                tests[0].iv.size());
        ...
        try
        {
            cryptor.initialize(
                ...
                tc::crypto::Aes128CtrEncryptor::kBlockSize - 1);
