...
crypto_Des56CbcEncryptor_TestClass::crypto_Des56CbcEncryptor_TestClass()
    : mTestTag("tc::crypto::Des56CbcEncryptor"), mTestResults()
{
}

void crypto_Des56CbcEncryptor_TestClass::runAllTests(void)
{
    test_Constants();
    test_UseClassEnc();
    test_UseClassDec();
...
const std::string &crypto_Des56CbcEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Des56CbcEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
...
void crypto_Des56CbcEncryptor_TestClass::test_Constants()
{
    ...
    try
    {
        ...
        if (tc::crypto::Des56CbcEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Des56CbcEncryptor::kBlockSize,
                kExpectedBlockSize));
        }

        // check key size
        static const size_t kExpectedKeySize = 8;
        if (tc::crypto::Des56CbcEncryptor::kKeySize != kExpectedKeySize)
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Des56CbcEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Des56CbcEncryptor_TestClass::test_UseClassEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        TdesDesCbcEncryptorUtil::generateDesCbcTestVectors_Nist(tests, 56);
        if (tests.begin() == tests.end())
        {
            throw tc::TestException("No test vectors");
        }

        tc::crypto::Des56CbcEncryptor cryptor;

        // The repeated iteration test for CBC needs extra steps because encryption XORs the IV with the input block,
        // and since after the first iteration, the IV and input block are the same, some alternation needs to be added
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            if (memcmp(data.data(), test->enc_data.data(), data.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    ...
                    tc::cli::FormatUtil::formatBytesAsString(test->enc_data, true, "")));
            }
        }
...
void crypto_Des56CbcEncryptor_TestClass::test_UseClassDec()
{
    TestResult test_result;
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            if (memcmp(data.data(), test->dec_data.data(), data.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(data, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->dec_data, true, "")));
            }
        }
...
void crypto_Des56CbcEncryptor_TestClass::test_UseUtilFuncEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            for (size_t itr = 0; itr < test->iteration_count; itr++)
            {
                tc::crypto::EncryptDes56Cbc(
                    data.data(), data.data(), data.size(), test->key.data(), test->key.size(), iv.data(), iv.size());
...
void crypto_Des56CbcEncryptor_TestClass::test_UseUtilFuncDec()
{
    TestResult test_result;
...
void crypto_Des56CbcEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Des56CbcEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Des56CbcEncryptor::kBlockSize);
...
void crypto_Des56CbcEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tc::crypto::Des56CbcEncryptor::kKeySize - 1, tests[0].iv.data(),
                tests[0].iv.size());
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tests[0].key.size(), tests[0].iv.data(),
                tc::crypto::Des56CbcEncryptor::kBlockSize - 1);
...
void crypto_Des56CbcEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        try
        {
            cryptor.encrypt(data.data(), tests[0].base_data.data(), tc::crypto::Des56CbcEncryptor::kBlockSize - 1);
...
void crypto_Des56CbcEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].base_data.data(), tc::crypto::Des56CbcEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
