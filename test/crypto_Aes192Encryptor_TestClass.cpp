...
crypto_Aes192Encryptor_TestClass::crypto_Aes192Encryptor_TestClass()
    : mTestTag("tc::crypto::Aes192Encryptor"), mTestResults()
{
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes192Encryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes192Encryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes192Encryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes192Encryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes192Encryptor::kBlockSize)
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes192Encryptor::kKeySize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Failed to throw correct exception where key_size==tc::crypto::Aes192Encryptor::kKeySize-1");
        }
