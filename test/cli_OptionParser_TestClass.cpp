...
cli_OptionParser_TestClass::cli_OptionParser_TestClass() : mTestTag("tc::cli::OptionParser"), mTestResults() {}
    ...
    try
    {
        ...
        catch (const tc::Exception &)
        {
            throw tc::TestException(
                ".processOptions() threw the wrong exception when an unhandled option was detected");
        }
...
void cli_OptionParser_TestClass::test_ProcessOptionsWithOnlyUnkHandler()
{
    ...
    try
    {
        ...
        {
            throw tc::TestException("Unrecognised option handler was registered but the option & parameter were not "
...
void cli_OptionParser_TestClass::test_ProcessOptionsWithLiteralHandlers()
{
    ...
    try
    {
        ...
        opt.registerOptionHandler(
            std::make_shared<TestOptionHandler>(TestOptionHandler(flag_result, "-flagoption", "", "-flagoption", {})));

        TestOptionHandler::ResultCode single_param_result = TestOptionHandler::ResultCode::DidNotUseOptionHandler;
        opt.registerOptionHandler(std::make_shared<TestOptionHandler>(
            TestOptionHandler(single_param_result, "-singleparam", "", "-singleparam", {"my_param"})));

        TestOptionHandler::ResultCode multi_param_result = TestOptionHandler::ResultCode::DidNotUseOptionHandler;
        opt.registerOptionHandler(std::make_shared<TestOptionHandler>(
            TestOptionHandler(multi_param_result, "-multiparam", "", "-multiparam", {"my_param1", "my_param2"})));

        std::vector<std::string> args = {"-flagoption", "-singleparam", "my_param",
                                         "-multiparam", "my_param1",    "my_param2"};
        ...
        {
            throw tc::TestException("Option handler (for flag option) was registered but the option & parameter were "
                                    "not passed to it correctly.");
        }
        ...
        {
            throw tc::TestException("Option handler (for single param option) was registered but the option & "
                                    "parameter were not passed to it correctly.");
        }
        ...
        {
            throw tc::TestException("Option handler (for multi param option) was registered but the option & parameter "
                                    "were not passed to it correctly.");
        }
...
void cli_OptionParser_TestClass::test_ProcessOptionsWithRegexHandlers()
{
    ...
    try
    {
        ...
        TestOptionHandler::ResultCode flag_result = TestOptionHandler::ResultCode::DidNotUseOptionHandler;
        opt.registerOptionHandler(
            std::make_shared<TestOptionHandler>(TestOptionHandler(flag_result, "", "(-F.a.+)", "-Flag", {})));

        TestOptionHandler::ResultCode single_param_result = TestOptionHandler::ResultCode::DidNotUseOptionHandler;
        opt.registerOptionHandler(std::make_shared<TestOptionHandler>(
            TestOptionHandler(single_param_result, "", "(-s.+)", "-singleparam", {"my_param"})));
...
void cli_OptionParser_TestClass::test_ProcessOptionsWithLiteralAndRegexHandlers()
{
    ...
    try
    {
        ...
        opt.registerOptionHandler(
            std::make_shared<TestOptionHandler>(TestOptionHandler(regex_result, "", "(-D.+)", "-DKEY", {"value"})));

        TestOptionHandler::ResultCode flag_result = TestOptionHandler::ResultCode::DidNotUseOptionHandler;
        opt.registerOptionHandler(
            std::make_shared<TestOptionHandler>(TestOptionHandler(flag_result, "-flag", "", "-flag", {})));
        ...
        std::vector<std::string> args = {"-flag",       "-DKEY=value", "-singleparam", "my_param",
                                         "-multiparam", "my_param1",   "my_param2"};
        ...
        {
            throw tc::TestException("Option handler (for regex option) was registered but the option & parameter were "
                                    "not passed to it correctly.");
        }
...
void cli_OptionParser_TestClass::test_NullHandlerSupplied()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".registerOptionHandler() did not throw ArgumentNullException when passed a nullptr.");
        }
        ...
        try
        {
            ...
            throw tc::TestException(
                ".registerUnrecognisedOptionHandler() did not throw ArgumentNullException when passed a nullptr.");
        }
        catch (const tc::ArgumentNullException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                ".registerUnrecognisedOptionHandler() threw the wrong exception when passed a nullptr.");
        }
...
void cli_OptionParser_TestClass::test_RegularHandlerProvidesNoOptionLiteralOrRegex()
{
    ...
    try
    {
        ...
        try
        {
            ...
            opt.registerOptionHandler(
                std::make_shared<TestOptionHandler>(TestOptionHandler(test_result, "", "", "-DKEY", {"value"})));
            throw tc::TestException(".registerOptionHandler() Did not throw tc::ArgumentOutOfRangeException when "
                                    "option handler had no option literals or option regex.");
        }
        ...
        {
            throw tc::TestException(".registerOptionHandler() threw the wrong exception when option handler had no "
                                    "option literals or option regex.");
        }
...
void cli_OptionParser_TestClass::test_ProcessMalformedOptions()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".processOptions() did not throw exception for dangling parameter located after compound option");
        }
        catch (const tc::ArgumentException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                ".processOptions() threw the wrong exception for dangling parameter located after compound option");
        }
