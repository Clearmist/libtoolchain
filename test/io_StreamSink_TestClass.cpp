...
io_StreamSink_TestClass::io_StreamSink_TestClass() : mTestTag("tc::io::StreamSink"), mTestResults() {}
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "Create Constructor did not throw tc::ArgumentNullException where base stream was null");
        }
...
void io_StreamSink_TestClass::testCreateFromStreamWithoutSeek()
{
    ...
    try
    {
        try
        {
            ...
            tc::io::StreamSink sink =
                tc::io::StreamSink(std::make_shared<StreamTestUtil::DummyStreamBase>(base_stream));

            throw tc::TestException(
                "Create Constructor did not throw tc::NotSupportedException where base stream did not support seeking");
        }
        ...
        {
            throw tc::TestException(
                "Create Constructor threw the wrong exception where base stream did not support seeking");
        }
...
void io_StreamSink_TestClass::testCreateFromStreamWithoutWrite()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "Create Constructor did not throw tc::NotSupportedException where base stream did not support writing");
        }
        catch (tc::NotSupportedException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Create Constructor threw the wrong exception where base stream did not support writing");
        }
...
void io_StreamSink_TestClass::testSetLengthOnDisposedBase()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "setLength() did not throw tc::ObjectDisposedException where StreamSink was disposed");
        }
...
void io_StreamSink_TestClass::testPushDataOnDisposedBase()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                ".pushData() did not throw tc::ObjectDisposedException where StreamSink was disposed");
        }
        catch (tc::ObjectDisposedException &)
        { /* do nothing */
        }
...
void io_StreamSink_TestClass::pushTestHelper(
    tc::io::ISink &sink,
    const std::shared_ptr<tc::io::IStream> &base_stream,
    ...
    int64_t push_offset)
{
    ...
    if (push_ret != expected_data.size())
    {
        throw tc::TestException(fmt::format(
            "pushData(offset: {:d}) returned: {:d}, when it should have been {:d}", push_offset, push_ret,
            expected_data.size()));
    }
    ...
    {
        throw tc::TestException(fmt::format(
            "pushData(offset: {:d}) failed to write enough bytes, position(): {:d}, when it should have been {:d}",
            push_offset, position_ret, expected_position));
    }
    ...
    {
        throw tc::TestException(fmt::format(
            "internal test method to adjust base_stream position failed. seek(offset: {:d}, origin: Begin): {:d}, when "
            "it should have been {:d}",
            push_offset, seek_ret, push_offset));
    }
    ...
    {
        throw tc::TestException(fmt::format(
            "internal test method to read from base_stream failed. read(size: {:d}): {:d}, when it should have been "
            "{:d}",
            expected_data.size(), read_ret, expected_data.size()));
    }

    if (memcmp(expected_data.data(), output_data.data(), expected_data.size()) != 0)
    {
        throw tc::TestException(
            fmt::format("pushData(offset: {:d}) did not write data to base_stream as expected", push_offset));
    }
}