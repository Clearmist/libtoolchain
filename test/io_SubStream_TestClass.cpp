...
io_SubStream_TestClass::io_SubStream_TestClass() : mTestTag("tc::io::SubStream"), mTestResults() {}
...
void io_SubStream_TestClass::testProperties()
{
    ...
    try
    {
        ...
        substream =
            std::shared_ptr<tc::io::SubStream>(new tc::io::SubStream(dummy_stream, substream_offset, substream_length));
        ...
        if (substream->canRead() != false)
        {
            throw tc::TestException(
                "canRead() returned true when base stream was valid, but basestream->canRead() was false.");
        }
        if (substream->canWrite() != true)
        {
            throw tc::TestException("canWrite() returned false when base stream was valid.");
        ...
        {
            throw tc::TestException(
                "canWrite() returned true when base stream was valid, but basestream->canWrite() was false.");
        }
        ...
        {
            throw tc::TestException(
                "canSeek() returned true when base stream was valid, but basestream->canSeek() was false.");
        }
        if (substream->canRead() != true)
        {
            throw tc::TestException("canRead() returned false when base stream was valid.");
...
void io_SubStream_TestClass::testSize()
{
    ...
    try
    {
        ...
        tc::io::SubStream substream(
            std::shared_ptr<DummyStream>(new DummyStream()), substream_offset, substream_length);
