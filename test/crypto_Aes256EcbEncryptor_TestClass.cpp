...
crypto_Aes256EcbEncryptor_TestClass::crypto_Aes256EcbEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes256EcbEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes256EcbEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes256EcbEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes256EcbEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes256EcbEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes256EcbEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes256EcbEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesEcbEncryptorUtil::generateAesEcbTestVectors_Nist(tests, 256);
...
void crypto_Aes256EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes256EcbEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes256EcbEncryptor::kBlockSize);
...
void crypto_Aes256EcbEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].plaintext.data(), tc::crypto::Aes256EcbEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
