...
{
  public:
    static void constructor_TestHelper(
    ...
    static void seek_TestHelper(
        tc::io::IStream &stream,
        int64_t seek_offset,
        tc::io::SeekOrigin seek_origin,
        int64_t exp_seek_res,
        int64_t exp_pos_res);
    static void read_TestHelper(
        tc::io::IStream &stream,
        int64_t seek_offset,
        tc::io::SeekOrigin seek_origin,
        size_t dst_size,
        ...
        size_t exp_read_res,
        int64_t exp_pos_res,
        const byte_t *expected_data = nullptr);
    static void write_TestHelper(
        tc::io::IStream &stream,
        int64_t seek_offset,
        tc::io::SeekOrigin seek_origin,
        tc::ByteData &data,
    ...
    {
      public:
        DummyStreamBase() : DummyStreamBase(0x10000000) {}

        DummyStreamBase(int64_t length) : DummyStreamBase(length, true, true, true, true, true) {}

        DummyStreamBase(
            int64_t length, bool canRead, bool canWrite, bool canSeek, bool canSeekOnlyFromBegin, bool canSetLength)
        {
        ...
        void
        init(int64_t length, bool canRead, bool canWrite, bool canSeek, bool canSeekOnlyFromBegin, bool canSetLength)
        {
        ...
        int64_t seek(int64_t offset, tc::io::SeekOrigin origin)
        {
            if (origin != tc::io::SeekOrigin::Begin && mCanSeekOnlyFromBegin)
                throw tc::ArgumentOutOfRangeException(
                    kClassName,
                    "Should not be passing seek origin values that are not SeekOrigin::Begin to the base stream");
