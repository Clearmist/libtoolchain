...
crypto_Aes128EcbEncryptor_TestClass::crypto_Aes128EcbEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes128EcbEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes128EcbEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes128EcbEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes128EcbEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128EcbEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128EcbEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes128EcbEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesEcbEncryptorUtil::generateAesEcbTestVectors_Nist(tests, 128);
...
void crypto_Aes128EcbEncryptor_TestClass::test_UseUtilFuncEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            tc::crypto::EncryptAes128Ecb(
                data.data(), test->plaintext.data(), data.size(), test->key.data(), test->key.size());
...
void crypto_Aes128EcbEncryptor_TestClass::test_UseUtilFuncDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            tc::crypto::DecryptAes128Ecb(
                data.data(), test->ciphertext.data(), data.size(), test->key.data(), test->key.size());
...
void crypto_Aes128EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes128EcbEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes128EcbEncryptor::kBlockSize);
...
void crypto_Aes128EcbEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].plaintext.data(), tc::crypto::Aes128EcbEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
