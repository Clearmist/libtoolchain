...
void runTest(
    ...
    const std::regex &include_test_regex,
    const std::regex &exclude_test_regex,
    bool run_slow_tests)
{
    ...
    if (!std::regex_match(test_class.getTestTag(), include_test_regex) ||
        std::regex_match(test_class.getTestTag(), exclude_test_regex))
    {
        return;
    }
...
void outputResultsToStdout(
    const std::vector<TestClassResult> &global_test_results,
    const std::regex &include_result_regex,
    const std::regex &exclude_result_regex)
{
    for (auto test_class_itr = global_test_results.begin(); test_class_itr != global_test_results.end();
         test_class_itr++)
    {
        size_t total_tests = 0, total_passed_tests = 0;
        fmt::print("[{:s}] START\n", test_class_itr->tag);
        for (auto test_itr = test_class_itr->test_results.begin(); test_itr != test_class_itr->test_results.end();
             test_itr++)
        {
            total_tests += 1;
            total_passed_tests += (test_itr->result == "PASS") ? 1 : 0;

            if (!std::regex_match(test_itr->result, include_result_regex) ||
                std::regex_match(test_itr->result, exclude_result_regex))
            {
                continue;
            }
...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
    {
        ...
        for (int i = 1; i < argc;)
        {
            ...
            catch (std::exception &e)
            {
                fmt::print("error: {}\n\n", e.what());
                fmt::print(
                    "usage: {} [{slowtest:s}] [{incltestregex:s} <regex>] [{excltestregex:s} <regex>] "
                    "[{inclresultregex:s} <regex>] [{exclresultregex:s} <regex>]\n",
                    std::string(argv[0]), fmt::arg("slowtest", kNoSlowTestFlag),
                    fmt::arg("incltestregex", kIncludeTestRegex), fmt::arg("excltestregex", kExcludeTestRegex),
                    fmt::arg("inclresultregex", kIncludeResultRegex), fmt::arg("exclresultregex", kExcludeResultRegex));
                fmt::print("\n\nTest Regex Examples:\n");
    ...
    runTest<string_TranscodeUtil_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    ...
    // namespace tc::encode
    runTest<encode_Base64Util_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    ...
    runTest<crypto_Sha1Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Sha2256Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Sha2512Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_HmacMd5Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_HmacSha1Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_HmacSha2256Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_HmacSha2512Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    ...
    runTest<crypto_Pbkdf2Sha1KeyDeriver_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Pbkdf2Sha2256KeyDeriver_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Pbkdf2Sha2512KeyDeriver_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    ...
    runTest<crypto_Aes128CcmEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_CmacAes128Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_CmacAes192Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_CmacAes256Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_CmacTdes112Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_CmacTdes168Generator_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Des56EcbEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Des56CbcEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Tdes112EcbEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Tdes112CbcEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Tdes168EcbEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Tdes168CbcEncryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa1024OaepSha2256Encryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048OaepSha2256Encryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096OaepSha2256Encryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048OaepSha2512Encryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096OaepSha2512Encryptor_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    ...
    runTest<crypto_Rsa4096Pkcs1Sha1Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa1024Pkcs1Sha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048Pkcs1Sha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096Pkcs1Sha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa1024Pkcs1Sha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048Pkcs1Sha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096Pkcs1Sha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa1024PssSha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048PssSha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096PssSha2256Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa1024PssSha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa2048PssSha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
    runTest<crypto_Rsa4096PssSha2512Signer_TestClass>(
        global_test_results, include_test_regex, exclude_test_regex, include_slow_tests);
