...
crypto_Rsa1024PssSha2256Signer_TestClass::crypto_Rsa1024PssSha2256Signer_TestClass()
    : mTestTag("tc::crypto::Rsa1024PssSha2256Signer"), mTestResults()
{
}

void crypto_Rsa1024PssSha2256Signer_TestClass::runAllTests(void)
{
    test_Constants();
    test_UseClassSign();
    test_UseClassVerify();
...
const std::string &crypto_Rsa1024PssSha2256Signer_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Rsa1024PssSha2256Signer_TestClass::getTestResults() const
{
    return mTestResults;
}
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_Constants()
{
    ...
    try
    {
        ...
        if (tc::crypto::Rsa1024PssSha2256Signer::kSignatureSize != kExpectedSignatureSize)
        {
            throw tc::TestException(fmt::format(
                "kSignatureSize had value {:d} (expected {:d}", tc::crypto::Rsa1024PssSha2256Signer::kSignatureSize,
                kExpectedSignatureSize));
        }
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_UseClassSign()
{
    TestResult test_result;
    ...
    try
    {
        ...
        tc::crypto::Rsa1024PssSha2256Signer signer;
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // initialize
            signer.initialize(tc::crypto::RsaPrivateKey(
            ...
            // test sign
            bool result =
                signer.sign(signature.data(), test->message_digest.data(), test->salt.data(), test->salt.size());
            if (result == false)
            {
            ...
            if (memcmp(signature.data(), test->signature.data(), signature.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    ...
                    tc::cli::FormatUtil::formatBytesAsString(signature, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->signature, true, "")));
            }
        }
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_UseClassVerify()
{
    TestResult test_result;
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_UseUtilFuncSign()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // signing using the util function does not support specifying the salt, so we'll have to do some `lite`
        // validation using the verify utility.
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            tc::ByteData signature = tc::ByteData(test->signature.size());
            ...
            // test sign
            bool result = tc::crypto::SignRsa1024PssSha2256(
                signature.data(), test->message_digest.data(),
            ...
            result = tc::crypto::VerifyRsa1024PssSha2256(
                signature.data(), test->message_digest.data(),
                tc::crypto::RsaPublicKey(
                    test->key_modulus.data(),
                    test->key_modulus
                        .size())); //(dec_data.data(), message_size, dec_data.size(), enc_data.data(),
                                   // tc::crypto::RsaPrivateKey(test->key_modulus.data(), test->key_modulus.size(),
                                   // test->key_private_exponent.data(), test->key_private_exponent.size()),
                                   // test->label.data(), test->label.size(), test->label_is_digested);
            if (result != true)
            {
                throw tc::TestException(
                    fmt::format("Test \"{:s}\" Failed: signature could not be verified", test->test_name));
            }
        }
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_UseUtilFuncVerify()
{
    TestResult test_result;
    ...
    try
    {
        ...
        {
            // test verify
            bool result = tc::crypto::VerifyRsa1024PssSha2256(
                test->signature.data(), test->message_digest.data(),
                tc::crypto::RsaPublicKey(test->key_modulus.data(), test->key_modulus.size()));
            if (result == false)
            {
                throw tc::TestException(fmt::format("Test \"{:s}\" Failed: verify() returned false", test->test_name));
            }
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_DoesNothingWhenNotInit()
{
    TestResult test_result;
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // signer.initialize(tc::crypto::RsaPrivateKey(tests[0].key_modulus.data(), tests[0].key_modulus.size(),
        // tests[0].key_private_exponent.data(), tests[0].key_private_exponent.size()));
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_SignReturnsFalseOnBadInput()
{
    TestResult test_result;
...
void crypto_Rsa1024PssSha2256Signer_TestClass::test_VerifyReturnsFalseOnBadInput()
{
    TestResult test_result;
