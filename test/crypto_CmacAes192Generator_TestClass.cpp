...
crypto_CmacAes192Generator_TestClass::crypto_CmacAes192Generator_TestClass()
    : mTestTag("tc::crypto::CmacAes192Generator"), mTestResults()
{
}
    ...
    try
    {
        ...
        if (tc::crypto::CmacAes192Generator::kMacSize != kExpectedMacSize)
        {
            throw tc::TestException(fmt::format(
                "kMacSize had value {:d} (expected {:d})", tc::crypto::CmacAes192Generator::kMacSize,
                kExpectedMacSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::CmacAes192Generator::kBlockSize,
                kExpectedBlockSize));
        }
...
void crypto_CmacAes192Generator_TestClass::test_SingleUpdateCall()
{
    ...
    try
    {
        ...
        for (auto test_case = test_cases.begin(); test_case != test_cases.end(); test_case++)
        {
            ...
            if (memcmp(
                    mac.data(), test_case->out_mac.data(), std::min<size_t>(mac.size(), test_case->out_mac.size())) !=
                0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed. Had wrong MAC: {:s} (expected {:s})", test_case->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(mac, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test_case->out_mac, true, "")));
            }
        }
...
void crypto_CmacAes192Generator_TestClass::test_UtilFunc()
{
    ...
    try
    {
        ...
        {
            memset(mac.data(), 0xff, mac.size());
            tc::crypto::GenerateCmacAes192Mac(
                mac.data(), test_case->in_data.data(), test_case->in_data.size(), test_case->in_key.data(),
                test_case->in_key.size());
            if (memcmp(
                    mac.data(), test_case->out_mac.data(), std::min<size_t>(mac.size(), test_case->out_mac.size())) !=
                0)
            {
...
void crypto_CmacAes192Generator_TestClass::test_NoInitNoUpdateDoMac()
{
    ...
    try
    {
        ...
        for (auto test_case = test_cases.begin(); test_case != test_cases.end(); test_case++)
        {
            ...
            calc.getMac(mac.data());
            if (memcmp(
                    mac.data(), expected_uninitialized_mac.data(),
                    std::min<size_t>(mac.size(), test_case->out_mac.size())) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed. Had wrong MAC: {:s} (expected {:s})", test_case->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(mac, true, ""),
...
void crypto_CmacAes192Generator_TestClass::test_InvalidKeyThrowsExceptionUtilFunc()
{
    ...
    try
    {
        ...
        {
            tc::crypto::GenerateCmacAes192Mac(
                mac.data(), test_cases[0].in_data.data(), test_cases[0].in_data.size(), test_cases[0].in_key.data(),
                test_cases[0].in_key.size() - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==kKeySize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        ...
        {
            tc::crypto::GenerateCmacAes192Mac(
                mac.data(), test_cases[0].in_data.data(), test_cases[0].in_data.size(), test_cases[0].in_key.data(), 0);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where key_size==0");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        ...
        {
            tc::crypto::GenerateCmacAes192Mac(
                mac.data(), test_cases[0].in_data.data(), test_cases[0].in_data.size(), nullptr,
                test_cases[0].in_key.size());
            throw tc::TestException("Failed to throw ArgumentNullException where key==nullptr");
        }
        catch (const tc::ArgumentNullException &)
        { /* do nothing */
    ...
    try
    {
        ...
        for (auto test_case = test_cases.begin(); test_case != test_cases.end(); test_case++)
        {
            ...
            for (size_t i = 0; i < 100; i++)
            {
                ...
                calc.getMac(mac.data());
                if (memcmp(
                        mac.data(), test_case->out_mac.data(),
                        std::min<size_t>(mac.size(), test_case->out_mac.size())) != 0)
                {
                    throw tc::TestException(fmt::format(
                        "Test \"{:s}\" Failed. Had wrong MAC: {:s} (expected {:s})", test_case->test_name,
...
void crypto_CmacAes192Generator_TestClass::util_Setup_TestCases(
    std::vector<crypto_CmacAes192Generator_TestClass::TestCase> &test_cases)
{
    TestCase tmp;
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes(
        "6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac45af8e5130c81c46a35ce411"); // Mlen 320-bit
    ...
    tmp.in_key = tc::cli::FormatUtil::hexStringToBytes("8e73b0f7da0e6452c810f32b809079e562f8ead2522c6b7b");
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes(
        "6bc1bee22e409f96e93d7e117393172aae2d8a571e03ac9c9eb76fac45af8e5130c81c46a35ce411e5fbc1191a0a52eff69f2445df4f9b"
        "17ad2b417be66c3710"); // Mlen 512-bit
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("86617012af7d947841ba408df5ff8b9a44f25985ccc5dc44f5525a97d46b30f7");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("a206a1eb70a9d24bb5e72f314e7d91de074f59055653bdd24aab5f2bbe112436");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("5e5136ff2c39c2cbb80746b3df2e39e2744c8f119a0238fb7002e911f4691e6e");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("f576f759b19cb7c6a35806859e066a5a632cd7b3b9476a8ac0cb5a8d82652b7d");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("402e802885e4119df17fe85f141c3d1af7727fcdb00f8e2c34e42a436d04ac5b");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("4b4e3dfbd10f464022fe105aa6365bdc3a700fdd2525cf414df3d03aab42ec0d");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("5787fcfe8a1ee76afaef54cb22d8b2a20b116f72bfc7117f010783d63bdeac8d");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("e98d6b26e408f0f1b44381bd131f43aaf2d660cb62480822b48fb6cbdfd9b577");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("3c3d7f18a0bd3b6f366c1b497c602e889cac88feedd7e683bd8d95454e958f60");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("6f6c7ce5d1c725f7b0284703736a1de9ca8f75cf9110b415f770198bdf1746e2");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("ca2b30d72df532efce9cde44ffb9bb5611e746ba1a76dc6af464d95f89880262");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("bed48837b7c36abfd299b0d12d0b0ee0c6330107982a0f3f07d950d47e0c2d1a");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("081568ae0b948aa647b9d4dda5d42641ad5de72aa9874d8d0717d872007720a8");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("d788790ec62797fe588f4a68ce4a00d329a8d7b39a65b22f3a868d27fb840d76");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("7bc03632942150716f019d048a752ccc0f93139c55df0f4aaa066a0550cf22e8");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("9be0b302ab1fe8d13cba71652b27be06f7f8e7f626b035ece4d5e4c3ee462774");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("ae8e93c9c991cf896a491a8907df4e4be5186ae496cd340dc19b237821db7b60");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes("bb525cbd4cfe784d33c4fa8b6e75006e256911b03ec4542f2cf8ff94078b1332");
    ...
    tmp.in_data =
        tc::cli::FormatUtil::hexStringToBytes

    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("d2e8a9c8f2cf5ac60c8c45dc8147050ed8bf67c26a35c1c85e868599d89715"
                                                        "dc6ef5d92fdcbd9023f8bae388e8ce40ec9293e55e54413b78f5d3669c");
    tmp.out_mac = tc::cli::FormatUtil::hexStringToBytes("973303b9a9555751cb12fa8f");
    test_cases.push_back(tmp);

    tmp.test_name = "CVAS 11.0 TestVector 129";
    tmp.in_key = tc::cli::FormatUtil::hexStringToBytes("a206a1eb70a9d24bb5e72f314e7d91de074f59055653bdd2");
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("a24666544c071b86072afb4db5532327708897d87b4dd6af780db10cdbbc5c"
                                                        "1f9ebbc6f237fc26aac766d60a947b6538b938e8c41c3c90e6a12d8207");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("fe6c4f52b7f2ef19a88156ea68ea5f4053a9e893cf18342bd4e28e35abb92f"
                                                        "17f54c6201f073e6c4f6ad94e846cd41f807f0e6218ab2fffcd0a230a7");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("565d9248960a8ba102a45c8e588840704a2d2391c65707fe7a8ea69598cf58"
                                                        "5ed681381fbaaa822a1463407acc8a26feddb6ba35a270b8fb51a0b4a3");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("ad46407517594cba4dec384ae7838768aa2139e5b6b39f9f00ab6f4803ec14"
                                                        "69b3e647a6b302e343c740f5e78c1e13a067bdf028bb3f2d675f737138");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("565011315f1755dd2f6b4e54e34424ee025cbacfb39186692d8ba0426ea018"
                                                        "f730f48d8219e5fbb832a6d5159a2954bc7c430eba3e0bca8bfb8393df");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("af39806b14c6db90478fa21852bd65285d2d9e1dddd28916085435b28cfddb"
                                                        "1b80d670439b0d766da49a17f029227437741c3b0dbb32771797eaada2");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("314f069dd4ac5aa3fdc2a74e83daa1d5d18330cd3b90684a9260bb48f5626d"
                                                        "49ebd7493fafda93abcb9e3d07e8f85d62e51ddf4a26ac77ee2fefda84");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("8ff821e97cef3fc38e19e2b95121186e6c63dcc1d351eb4f3d15d338d9784d"
                                                        "273c02cc8c9e62562c471193dc443324895ebe357c4ddd56dc28978675");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("d4a9a08cc1e1354c4265dd0451d10968a593c56b345a5356f7cc1317ed78a7"
                                                        "dc620b53e171c6aa86c82f67a5ab0799223c7d03c4f9a1c243861b30a2");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("9e60eb43006f5c54dedb4656e1fab0fe77d39c19c73a97d0ea3d8f2a58bfe5"
                                                        "d2a063c31b5dacb041656261f24337d20ce8aa96a99d2c6c593c8112ae");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("6a35dbc4a890ce23287efdeeff9042e7f931f26030e410a1ddcdc18a6f7300"
                                                        "7adac4936f325a3e6e78fc3625b468d8cf434f9b5873a244c14d0c59c7");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("e19ac08aeb3cd78e40548db43c5aca50ad7860fe923eeff02397068611fc8a"
                                                        "1f0ee75c30908b3693d2c2d6211c475f824525fc44766fc62b3032e8c8");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("8b4d85d9952650aeb2391f701f1d342b45e46f0a33b6671d4cb8b9593d32a0"
                                                        "e133f2c6844aeb5a86482263a38bcb545140aee046d6d00f3ab25092ee");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("d1de1be5f7a46191bb3d24d86fc3eabd5b6fd6655fb06fcbda7aa4b5cae10c"
                                                        "e734e67296c691901fbb401bf9ff3e00e89397ee74342b149a37022cc1");
    ...
    tmp.in_data = tc::cli::FormatUtil::hexStringToBytes("940cec23e887569aa81d5499025037be68db9ac7f28be806fd501e101994d2"
                                                        "b622cb7536ac4cd5b2a45fe5e03597c104e962ed1b11dedd5966eebbd6");
