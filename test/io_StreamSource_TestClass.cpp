...
io_StreamSource_TestClass::io_StreamSource_TestClass() : mTestTag("tc::io::StreamSource"), mTestResults() {}
...
void io_StreamSource_TestClass::testCreateFromStreamWithoutSeek()
{
    ...
    try
    {
        ...
        {
            // create source
            tc::io::StreamSource source = tc::io::StreamSource(std::shared_ptr<StreamTestUtil::DummyStreamBase>(
                new StreamTestUtil::DummyStreamBase(0x1000, true, true, false, true, true)));

            throw tc::TestException("Create Constructor failed to throw tc::NotSupportedException where base stream "
...
void io_StreamSource_TestClass::testCreateFromStreamWithoutRead()
{
    ...
    try
    {
        try
        {
            // create source
            tc::io::StreamSource source = tc::io::StreamSource(std::shared_ptr<StreamTestUtil::DummyStreamBase>(
                new StreamTestUtil::DummyStreamBase(0x1000, false, true, true, false, true)));

            throw tc::TestException("Create Constructor failed to throw tc::NotSupportedException where base stream "
                                    "did not support reading");
        }
        catch (tc::NotSupportedException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Create Constructor threw the wrong exception where base stream did not support reading");
        }
...
void io_StreamSource_TestClass::testCreateFromStreamWithoutWrite()
{
    ...
    try
    {
        // create source
        tc::io::StreamSource source = tc::io::StreamSource(std::shared_ptr<StreamTestUtil::DummyStreamBase>(
            new StreamTestUtil::DummyStreamBase(0x1000, true, false, true, false, true)));
