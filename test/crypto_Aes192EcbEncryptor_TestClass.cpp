...
crypto_Aes192EcbEncryptor_TestClass::crypto_Aes192EcbEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes192EcbEncryptor"), mTestResults()
{
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes192EcbEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes192EcbEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes192EcbEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes192EcbEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes192EcbEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes192EcbEncryptor::kBlockSize);
...
void crypto_Aes192EcbEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        try
        {
            cryptor.decrypt(data.data(), tests[0].plaintext.data(), tc::crypto::Aes192EcbEncryptor::kBlockSize - 1);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where size==kBlockSize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
