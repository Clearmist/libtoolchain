...
void SinkTestUtil::testSinkLength(tc::io::ISink &sink, int64_t expected_len)
{
    ...
    if (actual_len != expected_len)
    {
        throw tc::TestException(
            fmt::format("length() returned: {:d}, when it should have been {:d}.", actual_len, expected_len));
    }
}

SinkTestUtil::DummySinkBase::DummySinkBase() : DummySinkBase(0x10000000) {}

SinkTestUtil::DummySinkBase::DummySinkBase(int64_t length) : DummySinkBase(length, true) {}
...
SinkTestUtil::DummySinkTestablePushData::DummySinkTestablePushData()
    : DummySinkBase(), expected_data(std::make_shared<tc::ByteData>(0)), expected_offset(std::make_shared<int64_t>(0))
{
}
...
size_t SinkTestUtil::DummySinkTestablePushData::pushData(const tc::ByteData &data, int64_t offset)
{
    if (offset != *expected_offset)
    {
        throw tc::TestException(fmt::format(
            "pushData() was called on base_sink with offset {:d}, when it should have been {:d}.", offset,
            *expected_offset));
    }
    ...
    {
        throw tc::TestException(fmt::format(
            "pushData() passed a ByteData to base_sink with size {:d}, when it should have been {:d}.", data.size(),
            expected_data->size()));
    }
