...
io_SubSink_TestClass::io_SubSink_TestClass() : mTestTag("tc::io::SubSink"), mTestResults() {}
...
void io_SubSink_TestClass::testCreateConstructor()
{
    ...
    try
    {
        ...
        // create base sink
        auto base_sink =
            std::shared_ptr<SinkTestUtil::DummySinkTestablePushData>(new SinkTestUtil::DummySinkTestablePushData());
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "Create Constructor did not throw tc::ArgumentNullException where base sink was null");
        }
...
void io_SubSink_TestClass::testCreateWithNegativeSubSinkOffset()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "Create Constructor did not throw tc::ArgumentOutOfRangeException sub sink offset was negative");
        }
...
void io_SubSink_TestClass::testCreateWithNegativeSubSinkLength()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                "Create Constructor did not throw tc::ArgumentOutOfRangeException where sub sink size was negative");
        }
        catch (tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
...
void io_SubSink_TestClass::testCreateWithExcessiveSubSink()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException("Create Constructor did not throw tc::ArgumentOutOfRangeException where sub sink "
                                    "partially past the end of the base sink");
        }
        catch (tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Create Constructor threw the wrong exception where sub sink partially past the end of the base sink");
        }
...
void io_SubSink_TestClass::testCreateThenSetLength()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                ".setLength() did not throw tc::NotSupportedException where SubSink was initialised");
        }
...
void io_SubSink_TestClass::testSetLengthOnDisposedBase()
{
    ...
    try
    {
        try
        {
            ...
            throw tc::TestException(
                ".setLength() did not throw tc::ObjectDisposedException where SubSink was disposed");
        }
...
void io_SubSink_TestClass::testPushDataOutsideOfBaseRange()
{
    ...
    try
    {
        ...
        auto expected_data = tc::ByteData(
            push_data.data(), tc::io::IOUtil::getWritableCount(sub_sink_size, push_offset, push_data.size()));
...
void io_SubSink_TestClass::pushDataTestHelper(
    tc::io::ISink &sub_sink,
    const std::shared_ptr<SinkTestUtil::DummySinkTestablePushData> &base_sink,
    int64_t sub_base_offset,
    int64_t sub_push_offset,
    tc::ByteData &push_data,
    tc::ByteData &expected_data)
{
    ...
    if (push_ret != expected_data.size())
    {
        throw tc::TestException(fmt::format(
            "pushData(offset: {:d}) returned: {:d}, when it should have been {:d}", sub_push_offset, push_ret,
            expected_data.size()));
    }
}