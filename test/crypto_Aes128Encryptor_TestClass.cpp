...
crypto_Aes128Encryptor_TestClass::crypto_Aes128Encryptor_TestClass()
    : mTestTag("tc::crypto::Aes128Encryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes128Encryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes128Encryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes128Encryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128Encryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128Encryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes128Encryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes128Encryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes128Encryptor::kBlockSize);
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentOutOfRangeException where key_size==tc::crypto::Aes128Encryptor::kKeySize-1");
        }
        catch (const tc::ArgumentOutOfRangeException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Failed to throw correct exception where key_size==tc::crypto::Aes128Encryptor::kKeySize-1");
        }
