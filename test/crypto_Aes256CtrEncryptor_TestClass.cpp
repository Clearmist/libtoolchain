...
crypto_Aes256CtrEncryptor_TestClass::crypto_Aes256CtrEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes256CtrEncryptor"), mTestResults()
{
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes256CtrEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes256CtrEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes256CtrEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes256CtrEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes256CtrEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes256CtrEncryptor::kBlockSize);
...
void crypto_Aes256CtrEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tc::crypto::Aes256CtrEncryptor::kKeySize - 1, tests[0].iv.data(),
                tests[0].iv.size());
        ...
        try
        {
            cryptor.initialize(
                ...
                tc::crypto::Aes256CtrEncryptor::kBlockSize - 1);
