...
crypto_Aes128XtsEncryptor_TestClass::crypto_Aes128XtsEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes128XtsEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes128XtsEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes128XtsEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes128XtsEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128XtsEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128XtsEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes128XtsEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesXtsEncryptorUtil::generateAesXtsTestVectors_IEEE1619_2007(tests, 128);
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // initialize key
            cryptor.initialize(
                test->key1.data(), test->key1.size(), test->key2.data(), test->key2.size(), test->data_unit, true);
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            if (memcmp(data.data(), test->plaintext.data(), data.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(data, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->plaintext, true, "")));
            }
        }
...
void crypto_Aes128XtsEncryptor_TestClass::test_UseUtilFuncEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            tc::crypto::EncryptAes128Xts(
                data.data(), test->plaintext.data(), data.size(), test->data_unit_sequence_number, test->key1.data(),
                test->key1.size(), test->key2.data(), test->key2.size(), test->data_unit, true);
...
void crypto_Aes128XtsEncryptor_TestClass::test_UseUtilFuncDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            tc::crypto::DecryptAes128Xts(
                data.data(), test->ciphertext.data(), data.size(), test->data_unit_sequence_number, test->key1.data(),
                test->key1.size(), test->key2.data(), test->key2.size(), test->data_unit, true);
...
void crypto_Aes128XtsEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes128XtsEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes128XtsEncryptor::kBlockSize);
...
void crypto_Aes128XtsEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        // cryptor.initialize(tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(), tests[0].key2.size(),
        // tests[0].data_unit, true);

        try
        {
            cryptor.initialize(
                nullptr, tests[0].key1.size(), tests[0].key2.data(), tests[0].key2.size(), tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), 0, tests[0].key2.data(), tests[0].key2.size(), tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tc::crypto::Aes128XtsEncryptor::kKeySize - 1, tests[0].key2.data(),
                tests[0].key2.size(), tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tests[0].key1.size(), nullptr, tests[0].key2.size(), tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(), 0, tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(),
                tc::crypto::Aes128XtsEncryptor::kKeySize - 1, tests[0].data_unit, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(), tests[0].key2.size(), 0, true);
        ...
        {
            cryptor.initialize(
                tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(), tests[0].key2.size(),
                tc::crypto::Aes128XtsEncryptor::kBlockSize - 1, true);
...
void crypto_Aes128XtsEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        cryptor.initialize(
            tests[0].key1.data(), tests[0].key1.size(), tests[0].key2.data(), tests[0].key2.size(), tests[0].data_unit,
            true);
