...
crypto_Aes192CtrEncryptor_TestClass::crypto_Aes192CtrEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes192CtrEncryptor"), mTestResults()
{
}
...
const std::string &crypto_Aes192CtrEncryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Aes192CtrEncryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes192CtrEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes192CtrEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes192CtrEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes192CtrEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        AesCtrEncryptorUtil::generateAesCtrTestVectors_Nist(tests, 192);
...
void crypto_Aes192CtrEncryptor_TestClass::test_UseUtilFuncEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            tc::crypto::EncryptAes192Ctr(
                data.data(), test->plaintext.data(), data.size(), test->block_number, test->key.data(),
                test->key.size(), test->iv.data(), test->iv.size());
...
void crypto_Aes192CtrEncryptor_TestClass::test_UseUtilFuncDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            tc::crypto::DecryptAes192Ctr(
                data.data(), test->ciphertext.data(), data.size(), test->block_number, test->key.data(),
                test->key.size(), test->iv.data(), test->iv.size());
...
void crypto_Aes192CtrEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        // create data
        tc::ByteData control_data = tc::io::PaddingSource(0xee, tc::crypto::Aes192CtrEncryptor::kBlockSize)
                                        .pullData(0, tc::crypto::Aes192CtrEncryptor::kBlockSize);
...
void crypto_Aes192CtrEncryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        {
            cryptor.initialize(
                tests[0].key.data(), tc::crypto::Aes192CtrEncryptor::kKeySize - 1, tests[0].iv.data(),
                tests[0].iv.size());
        ...
        try
        {
            cryptor.initialize(
                ...
                tc::crypto::Aes192CtrEncryptor::kBlockSize - 1);
