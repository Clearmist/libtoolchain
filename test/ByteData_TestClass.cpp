...
ByteData_TestClass::ByteData_TestClass() : mTestTag("tc::ByteData"), mTestResults() {}
    ...
    try
    {
        ...
        if (data.data() != nullptr)
        {
            throw tc::TestException(
                ".data() did not return nullptr when ByteData was constructed with default constructor");
        }
...
void ByteData_TestClass::test_Constructor_InitializerList()
{
    ...
    try
    {
        ...
        byte_t expected_data[expected_data_size] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
        ...
        if (data.size() != expected_data_size)
        {
            throw tc::TestException(fmt::format(
                ".size() did not return {:d} when ByteData was constructed with an initializer list",
                expected_data_size));
        }
...
void ByteData_TestClass::test_Constructor_CreateSmallSized()
{
    ...
    try
    {
        ...
        {
            throw tc::TestException(
                fmt::format(".data() returned nullptr when ByteData was constructed with size {:d}", data_size));
        }

        if (data.size() != data_size)
        {
            throw tc::TestException(fmt::format(
                ".size() did not return {:d} (returned {:d}) when ByteData was constructed with size {:d}", data_size,
                data.size(), data_size));
        }
...
void ByteData_TestClass::test_Constructor_CreateFromPtr()
{
    ...
    try
    {
        ...
        {
            throw tc::TestException(
                fmt::format(".data() returned nullptr when ByteData was constructed with size {:d}", src_data_size));
        }

        if (data.size() != src_data_size)
        {
            throw tc::TestException(fmt::format(
                ".size() did not return {:d} (returned {:d}) when ByteData was constructed with size {:d}",
                src_data_size, data.size(), src_data_size));
        }
...
void ByteData_TestClass::test_EqualityOperator()
{
    ...
    try
    {
        ...
        const byte_t kControlData[kControlDataSize] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
        ...
        // test 2 - truncated ByteData
        {
            ...
            {
                throw tc::TestException(
                    "operator==(control, test) returned true for when compared to truncated ByteData");
            }
            if ((smaller == control_data) != false)
            {
                throw tc::TestException(
                    "operator==(test, control) returned true for when compared to truncated ByteData");
            }
        }
        ...
        // test 3 - same size different ByteData
        {
            ...
            {
                throw tc::TestException(
                    "operator==(control, test) returned true for when compared to same size but different ByteData");
            }
            if ((different == control_data) != false)
            {
                throw tc::TestException(
                    "operator==(test, control) returned true for when compared to same size but different ByteData");
            }
        }
        ...
        // test 5 - null to null
        {
            ...
            {
                throw tc::TestException(
                    "operator==(empty, empty2) returned true for when comparing two empty ByteData");
            }
            if ((empty_data == empty_data2) != true)
            {
                throw tc::TestException(
                    "operator==(empty2, empty) returned true for when comparing two empty ByteData");
            }
        }
