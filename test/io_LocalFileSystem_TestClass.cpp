...
io_LocalFileSystem_TestClass::io_LocalFileSystem_TestClass() : mTestTag("tc::io::LocalFileSystem"), mTestResults() {}
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".removeFile() with path that does not exist did not throw tc::io::FileNotFoundException");
        }
        ...
        {
            throw tc::TestException(
                fmt::format(".removeFile() with path that does not exist threw the wrong exception ({:s})", e.what()));
        }
...
void io_LocalFileSystem_TestClass::test_RemoveDirectory_NotExist()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".removeDirectory() with path that does not exist did not throw tc::io::DirectoryNotFoundException");
        }
        ...
        catch (const tc::Exception &e)
        {
            throw tc::TestException(fmt::format(
                ".removeDirectory() with path that does not exist threw the wrong exception ({:s})", e.what()));
        }
    ...
    try
    {
        ...
        try
        {
            try
            {
                ...
                throw tc::TestException(".removeDirectory() with path to a directory not empty did not throw "
                                        "tc::io::DirectoryNotEmptyException");
            }
            catch (const tc::io::DirectoryNotEmptyException &)
            { /* do nothing */
            }
            catch (const tc::Exception &e)
            {
                throw tc::TestException(fmt::format(
                    ".removeDirectory() with path to a directory not empty threw the wrong exception ({:s})",
                    e.what()));
            }
    ...
    try
    {
        ...
        try
        {
            try
            {
                ...
                throw tc::TestException(
                    ".removeDirectory() with path to a file did not throw tc::io::DirectoryNotFoundException");
            }
            catch (const tc::io::DirectoryNotFoundException &)
            { /* do nothing */
            }
            ...
            {
                throw tc::TestException(
                    fmt::format(".removeDirectory() with path to a file threw the wrong exception ({:s})", e.what()));
            }
...
void io_LocalFileSystem_TestClass::test_GetDirectoryListing_NotExist()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".getDirectoryListing() with invalid path did not throw tc::io::DirectoryNotFoundException");
        }
        catch (const tc::io::DirectoryNotFoundException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                fmt::format(".getDirectoryListing() with invalid path threw the wrong exception ({:s})", e.what()));
        }
...
void io_LocalFileSystem_TestClass::test_ChangeWorkingDirectory_NotExist()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException(
                ".setWorkingDirectory() with invalid path did not throw tc::io::DirectoryNotFoundException");
        }
        catch (const tc::io::DirectoryNotFoundException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                fmt::format(".setWorkingDirectory() with invalid path threw the wrong exception ({:s})", e.what()));
        }
...
void io_LocalFileSystem_TestClass::test_CreateDirectoryPath_NotExist()
{
    ...
    try
    {
        ...
        std::vector<std::string> created_dir_paths = {
            "./a/path/with/many/elements",
            "./a/path/with/many/hey",
            "./a/path/with/many/oi",
            "./a/path/with/many",
            "./a/path/with",
            "./a/path",
...
void io_LocalFileSystem_TestClass::test_CreateDirectoryPath_UnicodePath()
{
    ...
    try
    {
        ...
        std::vector<std::string> created_dir_paths = {
            "./ЀЁЂЃЄЅ/מבחן/тест/テスト/පරීක්ෂණය",
            "./ЀЁЂЃЄЅ/מבחן/тест/テスト/测试",
            "./ЀЁЂЃЄЅ/מבחן/тест/テスト/ਟੈਸਟ",
            "./ЀЁЂЃЄЅ/מבחן/тест/テスト",
            "./ЀЁЂЃЄЅ/מבחן/тест",
            "./ЀЁЂЃЄЅ/מבחן",
            "./ЀЁЂЃЄЅ"};
...
void io_LocalFileSystem_TestClass::test_GetCanonicalPath_DoesExist()
{
     ...
     * * it's important that both files & directories are tested, incase the impl for finding canonical path is just
     * set/get directory 4) generate some relative/absolute paths that will resolve to the canonical path as tests 5)
     * run those tests against getCanonicalPath()
     */
    ...
    try
    {
        ...
        try
        {
            for (auto itr = canonical_path_tests.begin(); itr != canonical_path_tests.end(); itr++)
            {
                ...
                {
                    throw tc::TestException(fmt::format(
                        "Failed to translate \"{:s}\" to \"{:s}\" ({:s})", itr->in_path.to_string(),
...
void io_LocalFileSystem_TestClass::test_GetCanonicalPath_NotExist()
{
     ...
     * 3) create some directories/files under the base directory using canonical paths
     * * it's important that both files & directories are tested, incase the impl for finding canonical path is just
     * set/get directory 4) generate some tests that contain invalid paths that are close to the real paths 5) run those
     * tests against getCanonicalPath() (they should all fail)
     */
        ...
        try
        {
            for (auto itr = canonical_phony_path_tests.begin(); itr != canonical_phony_path_tests.end(); itr++)
            {
                try
                {
                    ...
                    throw tc::TestException(fmt::format(
                        ".getCanonicalPath() failed to throw tc::io::DirectoryNotFoundException for invalid path "
                        "\"{:s}\"",
                        itr->to_string()));
                }
                ...
                {
                    throw tc::TestException(fmt::format(
                        ".getCanonicalPath() threw the wrong exception for invalid path \"{:s}\"", itr->to_string()));
                }
            }
