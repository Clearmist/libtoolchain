...
crypto_Aes128CcmEncryptor_TestClass::crypto_Aes128CcmEncryptor_TestClass()
    : mTestTag("tc::crypto::Aes128CcmEncryptor"), mTestResults()
{
}
    ...
    try
    {
        ...
        if (tc::crypto::Aes128CcmEncryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d})", tc::crypto::Aes128CcmEncryptor::kBlockSize,
                kExpectedBlockSize));
        }
        ...
        {
            throw tc::TestException(fmt::format(
                "kKeySize had value {:d} (expected {:d})", tc::crypto::Aes128CcmEncryptor::kKeySize, kExpectedKeySize));
        }
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseClassEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            cryptor.encrypt(
                payload.data(), test->plaintext.data(), payload.size(), test->nonce.data(), test->nonce.size(),
                test->aad.data(), test->aad.size(), mac.data(), mac.size());

            // validate cipher text
            if (memcmp(payload.data(), test->ciphertext.data(), payload.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: Ciphertext {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(payload, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->ciphertext, true, "")));
            }
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: MAC {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(mac, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->mac, true, "")));
            }
        }
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseClassDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            cryptor.decrypt(
                payload.data(), test->ciphertext.data(), payload.size(), test->nonce.data(), test->nonce.size(),
                test->aad.data(), test->aad.size(), mac.data(), mac.size());
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: Plaintext {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(payload, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->ciphertext, true, "")));
            }
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseClassDecVerify()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            bool decrypt_verify_result = cryptor.decrypt_and_verify(
                payload.data(), test->ciphertext.data(), payload.size(), test->nonce.data(), test->nonce.size(),
                test->aad.data(), test->aad.size(), test->mac.data(), test->mac.size());

            // confirm result was true (indicating valid mac and other params)
            if (decrypt_verify_result != true)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: decrypt_and_verify() returned {} (expected {})", test->test_name,
                    decrypt_verify_result, true));
            }
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseUtilFuncEnc()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // encrypt data
            tc::crypto::EncryptAes128Ccm(
                payload.data(), test->plaintext.data(), payload.size(), test->key.data(), test->key.size(),
                test->nonce.data(), test->nonce.size(), test->aad.data(), test->aad.size(), mac.data(), mac.size());
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseUtilFuncDec()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            tc::crypto::DecryptAes128Ccm(
                payload.data(), test->ciphertext.data(), payload.size(), test->key.data(), test->key.size(),
                test->nonce.data(), test->nonce.size(), test->aad.data(), test->aad.size(), mac.data(), mac.size());
...
void crypto_Aes128CcmEncryptor_TestClass::test_UseUtilFuncDecVerify()
{
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // decrypt data
            bool decrypt_verify_result = tc::crypto::DecryptVerifyAes128Ccm(
                payload.data(), test->ciphertext.data(), payload.size(), test->key.data(), test->key.size(),
                test->nonce.data(), test->nonce.size(), test->aad.data(), test->aad.size(), test->mac.data(),
                test->mac.size());
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: DecryptVerifyAes128Ccm() returned {} (expected {})", test->test_name,
                    decrypt_verify_result, true));
            }
...
void crypto_Aes128CcmEncryptor_TestClass::test_DoesNothingWhenNotInit()
{
    ...
    try
    {
        ...
        tc::ByteData control_ciphertext =
            tc::io::PaddingSource(0xbb, control_plaintext.size()).pullData(0, control_plaintext.size());
        ...
        cryptor.decrypt(
            plaintext.data(), ciphertext.data(), plaintext.size(), nonce.data(), nonce.size(), aad.data(), aad.size(),
            mac.data(), mac.size());
...
void crypto_Aes128CcmEncryptor_TestClass::test_EncryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        // cryptor.encrypt(payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
        // tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());

        try
        {
            cryptor.encrypt(
                nullptr, tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        {
            cryptor.encrypt(
                payload.data(), nullptr, payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), 0, tests[0].nonce.data(), tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), nullptr, tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), 0,
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                nullptr, aad.size(), mac.data(), mac.size());
        ...
        try
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), nullptr, mac.size());
        ...
        try
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                tests[0].aad.data(), tests[0].aad.size(), mac.data(), 0);
        ...
        for (size_t i = 0; i < invalid_iv_sizes.size(); i++)
        {
            ...
            {
                cryptor.encrypt(
                    payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                    invalid_iv_sizes[i], tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());

                throw tc::TestException(
                    fmt::format("Failed to throw ArgumentOutOfRangeException where iv_size=={}", invalid_iv_sizes[i]));
            }
            catch (const tc::ArgumentOutOfRangeException &)
            { /* do nothing */
            }
            ...
            {
                throw tc::TestException(
                    fmt::format("Failed to throw correct exception where iv_size=={}", invalid_iv_sizes[i]));
            }
        }
        ...
        for (size_t i = 0; i < valid_iv_sizes.size(); i++)
        {
            ...
            if (max_payload_size >= native_sizet_max)
                continue;

            try
            {
                cryptor.encrypt(
                    payload.data(), tests[0].plaintext.data(), size_t(max_payload_size) + 1, tests[0].nonce.data(),
                    valid_iv_sizes[i], tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());

                throw tc::TestException(fmt::format(
                    "Failed to throw ArgumentOutOfRangeException where size > 0x{:x} given iv_size == 0x{:x}",
                    max_payload_size, valid_iv_sizes[i]));
            }
            catch (const tc::ArgumentOutOfRangeException &)
            { /* do nothing */
            }
            ...
            {
                throw tc::TestException(fmt::format(
                    "Failed to throw correct exception where size > 0x{:x} given iv_size == 0x{:x}", max_payload_size,
                    valid_iv_sizes[i]));
            }
        }
        ...
        try
        {
            cryptor.encrypt(
                payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(), tests[0].nonce.size(),
                aad.data(), 0xff01, mac.data(), mac.size());
        ...
        for (size_t i = 0; i < invalid_mac_sizes.size(); i++)
        {
            try
            {
                cryptor.encrypt(
                    payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                    tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), invalid_mac_sizes[i]);

                throw tc::TestException(fmt::format(
                    "Failed to throw ArgumentOutOfRangeException where tag_size=={}", invalid_mac_sizes[i]));
            }
            catch (const tc::ArgumentOutOfRangeException &)
            { /* do nothing */
            }
            ...
            {
                throw tc::TestException(
                    fmt::format("Failed to throw correct exception where tag_size=={}", invalid_mac_sizes[i]));
            }
        }
...
void crypto_Aes128CcmEncryptor_TestClass::test_DecryptThrowsExceptionOnBadInput()
{
    ...
    try
    {
        ...
        // cryptor.decrypt(payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
        // tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());
        ...
        for (size_t i = 0; i < invalid_iv_sizes.size(); i++)
        {
            ...
            {
                throw tc::TestException(
                    fmt::format("Failed to throw correct exception where iv_size=={:d}", invalid_iv_sizes[i]));
            }
        }
...
void crypto_Aes128CcmEncryptor_TestClass::test_DecryptVerifyClassReturnsFalseOnBadInput()
{
    ...
    try
    {
        ...
        // reference decrypt call
        // cryptor.decrypt_and_verify(payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
        // tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size());

        if (false != cryptor.decrypt_and_verify(
                         nullptr, tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size()))
        {
        ...
        if (false != cryptor.decrypt_and_verify(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), nullptr, aad.size(), mac.data(), mac.size()))
        {
        ...
        if (false != cryptor.decrypt_and_verify(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), aad.data(), 0, mac.data(), mac.size()))
        {
            throw tc::TestException("decrypt_and_verify() did not return false where add!=nullptr but add_size==0");
        }
        ...
        if (false != cryptor.decrypt_and_verify(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), nullptr, mac.size()))
        {
        ...
        if (false != cryptor.decrypt_and_verify(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(), 0))
        {
            throw tc::TestException("decrypt_and_verify() did not return false where tag!=nullptr but tag_size==0");
        }
        ...
        for (size_t i = 0; i < invalid_iv_sizes.size(); i++)
        {
            ...
            {
                throw tc::TestException(
                    fmt::format("decrypt_and_verify() did not return false where iv_size=={}", invalid_iv_sizes[i]));
            }
        }
        ...
        for (size_t i = 0; i < valid_iv_sizes.size(); i++)
        {
            ...
            if (false != cryptor.decrypt_and_verify(
                             payload.data(), tests[0].plaintext.data(), size_t(max_payload_size) + 1,
                             tests[0].nonce.data(), valid_iv_sizes[i], tests[0].aad.data(), tests[0].aad.size(),
                             mac.data(), mac.size()))
            {
                throw tc::TestException(fmt::format(
                    "decrypt_and_verify() did not return false where size > 0x{:x} given iv_size == 0x{:x}",
                    max_payload_size, valid_iv_sizes[i]));
            }
        }
        ...
        if (false != cryptor.decrypt_and_verify(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                         tests[0].nonce.size(), aad.data(), 0xff01, mac.data(), mac.size()))
        {
        ...
        for (size_t i = 0; i < invalid_mac_sizes.size(); i++)
        {
            if (false != cryptor.decrypt_and_verify(
                             payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].nonce.data(),
                             tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(), mac.data(),
                             invalid_mac_sizes[i]))
            {
                throw tc::TestException(
                    fmt::format("decrypt_and_verify() did not return false where tag_size=={}", invalid_mac_sizes[i]));
            }
        }
...
void crypto_Aes128CcmEncryptor_TestClass::test_DecryptVerifyUtilFuncReturnsFalseOnBadInput()
{
    ...
    try
    {
        ...
        // reference decrypt call
        // tc::crypto::DecryptVerifyAes128Ccm(payload.data(), tests[0].plaintext.data(), payload.size(),
        // tests[0].key.data(), tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(),
        // tests[0].aad.size(), mac.data(), mac.size());

        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), nullptr, tests[0].key.size(),
                         tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(), 0,
                         tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tc::crypto::Aes128CcmEncryptor::kKeySize - 1, tests[0].nonce.data(), tests[0].nonce.size(),
                         tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where key_size==kKeySize-1");
        }
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         nullptr, tests[0].plaintext.data(), payload.size(), tests[0].key.data(), tests[0].key.size(),
                         tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), nullptr, payload.size(), tests[0].key.data(), tests[0].key.size(),
                         tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), 0, tests[0].key.data(), tests[0].key.size(),
                         tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), nullptr, tests[0].nonce.size(), tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where iv==nullptr but iv_size!=0");
        }

        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), 0, tests[0].aad.data(), tests[0].aad.size(),
                         mac.data(), mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where iv!=nullptr but iv_size==0");
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), nullptr, aad.size(),
                         mac.data(), mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where add==nullptr but add_size!=0");
        }

        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), aad.data(), 0, mac.data(),
                         mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where add!=nullptr but add_size==0");
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(),
                         tests[0].aad.size(), nullptr, mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where tag==nullptr but tag_size!=0");
        }

        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(),
                         tests[0].aad.size(), mac.data(), 0))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where tag!=nullptr but tag_size==0");
        }
        ...
        for (size_t i = 0; i < invalid_iv_sizes.size(); i++)
        {
            if (false != tc::crypto::DecryptVerifyAes128Ccm(
                             payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                             tests[0].key.size(), tests[0].nonce.data(), invalid_iv_sizes[i], tests[0].aad.data(),
                             tests[0].aad.size(), mac.data(), mac.size()))
            {
                throw tc::TestException(fmt::format(
                    "tc::crypto::DecryptVerifyAes128Ccm() did not return false where iv_size=={}",
                    invalid_iv_sizes[i]));
            }
        }
        ...
        for (size_t i = 0; i < valid_iv_sizes.size(); i++)
        {
            ...
            if (false != tc::crypto::DecryptVerifyAes128Ccm(
                             payload.data(), tests[0].plaintext.data(), size_t(max_payload_size) + 1,
                             tests[0].key.data(), tests[0].key.size(), tests[0].nonce.data(), valid_iv_sizes[i],
                             tests[0].aad.data(), tests[0].aad.size(), mac.data(), mac.size()))
            {
                throw tc::TestException(fmt::format(
                    "tc::crypto::DecryptVerifyAes128Ccm() did not return false where size > 0x{:x} given iv_size == "
                    "0x{:x}",
                    max_payload_size, valid_iv_sizes[i]));
            }
        }
        ...
        if (false != tc::crypto::DecryptVerifyAes128Ccm(
                         payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                         tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), aad.data(), 0xff01,
                         mac.data(), mac.size()))
        {
            throw tc::TestException(
                "tc::crypto::DecryptVerifyAes128Ccm() did not return false where add_size > 0xff00");
        }
        ...
        for (size_t i = 0; i < invalid_mac_sizes.size(); i++)
        {
            if (false != tc::crypto::DecryptVerifyAes128Ccm(
                             payload.data(), tests[0].plaintext.data(), payload.size(), tests[0].key.data(),
                             tests[0].key.size(), tests[0].nonce.data(), tests[0].nonce.size(), tests[0].aad.data(),
                             tests[0].aad.size(), mac.data(), invalid_mac_sizes[i]))
            {
                throw tc::TestException(fmt::format(
                    "tc::crypto::DecryptVerifyAes128Ccm() did not return false where tag_size=={}",
                    invalid_mac_sizes[i]));
            }
        }
