...
crypto_Rsa4096OaepSha2256Encryptor_TestClass::crypto_Rsa4096OaepSha2256Encryptor_TestClass()
    : mTestTag("tc::crypto::Rsa4096OaepSha2256Encryptor"), mTestResults()
{
}

void crypto_Rsa4096OaepSha2256Encryptor_TestClass::runAllTests(void)
{
    test_Constants();
    test_UseClassDec();
    test_UseClassEnc();
...
const std::string &crypto_Rsa4096OaepSha2256Encryptor_TestClass::getTestTag() const
{
    return mTestTag;
}

const std::vector<ITestClass::TestResult> &crypto_Rsa4096OaepSha2256Encryptor_TestClass::getTestResults() const
{
    return mTestResults;
}
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_Constants()
{
    ...
    try
    {
        ...
        if (tc::crypto::Rsa4096OaepSha2256Encryptor::kBlockSize != kExpectedBlockSize)
        {
            throw tc::TestException(fmt::format(
                "kBlockSize had value {:d} (expected {:d}", tc::crypto::Rsa4096OaepSha2256Encryptor::kBlockSize,
                kExpectedBlockSize));
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_UseClassDec()
{
    TestResult test_result;
    ...
    try
    {
        ...
        tc::crypto::Rsa4096OaepSha2256Encryptor cryptor;
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // initialize
            cryptor.initialize(
                tc::crypto::RsaPrivateKey(
                    test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(),
                    test->key_private_exponent.size()),
                test->label.data(), test->label.size(), test->label_is_digested);
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: message_size = {:d} (expected {:d})", test->test_name, message_size,
                    test->dec_message.size()));
            }
            if (memcmp(data.data(), test->dec_message.data(), data.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(data, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->dec_message, true, "")));
            }
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_UseClassEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // initialize
            cryptor.initialize(
                tc::crypto::RsaPublicKey(test->key_modulus.data(), test->key_modulus.size()), test->label.data(),
            ...
            // test encryption
            cryptor.encrypt(
                data.data(), test->dec_message.data(), test->dec_message.size(), test->enc_seed.data(),
                test->enc_seed.size());
            if (memcmp(data.data(), test->enc_message.data(), data.size()) != 0)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(data, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->enc_message, true, "")));
            }
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_UseUtilFuncDec()
{
    TestResult test_result;
    ...
    try
    {
        ...
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // test decryption
            size_t message_size = 0;
            bool res = tc::crypto::DecryptRsa4096OaepSha2256(
                data.data(), message_size, data.size(), test->enc_message.data(),
                tc::crypto::RsaPrivateKey(
                    test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(),
                    test->key_private_exponent.size()),
                test->label.data(), test->label.size(), test->label_is_digested);
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: DecryptRsa4096OaepSha2256 returned {} (expected {})", test->test_name, res,
                    true));
            }
            if (message_size != test->dec_message.size())
            {
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_UseUtilFuncEnc()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // the utility function doesn't support specifying a seed, so we'll have to do some `lite` validation using the
        // already validated decryption utility.
        for (auto test = tests.begin(); test != tests.end(); test++)
        {
            ...
            // test encryption
            bool res = tc::crypto::EncryptRsa4096OaepSha2256(
                enc_data.data(), test->dec_message.data(), test->dec_message.size(),
                tc::crypto::RsaPublicKey(test->key_modulus.data(), test->key_modulus.size()), test->label.data(),
                test->label.size(), test->label_is_digested);
            if (res != true)
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: EncryptRsa4096OaepSha2256 returned {} (expected {})", test->test_name, res,
                    true));
            }

            // try to decrypt message
            size_t message_size = 0;
            res = tc::crypto::DecryptRsa4096OaepSha2256(
                dec_data.data(), message_size, dec_data.size(), enc_data.data(),
                tc::crypto::RsaPrivateKey(
                    test->key_modulus.data(), test->key_modulus.size(), test->key_private_exponent.data(),
                    test->key_private_exponent.size()),
                test->label.data(), test->label.size(), test->label_is_digested);
            if (res != true)
            {
                throw tc::TestException(
                    fmt::format("Test \"{:s}\" Failed: encrypted message could not be decrypted", test->test_name));
            }
            if (message_size != test->dec_message.size())
            {
                throw tc::TestException(fmt::format(
            ...
            {
                throw tc::TestException(fmt::format(
                    "Test \"{:s}\" Failed: decrypted message was not expected {:s} (expected {:s})", test->test_name,
                    tc::cli::FormatUtil::formatBytesAsString(dec_data, true, ""),
                    tc::cli::FormatUtil::formatBytesAsString(test->dec_message, true, "")));
            }
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_UnspecifiedSeedProducesDifferentBlock()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // initialize
        cryptor.initialize(
            tc::crypto::RsaPublicKey(tests[0].key_modulus.data(), tests[0].key_modulus.size()), tests[0].label.data(),
            tests[0].label.size(), tests[0].label_is_digested);
        ...
        for (size_t i = 0; i < kTestNum; i++)
        {
            ...
            {
                difference_score +=
                    memcmp(enc_messages[i].data(), enc_messages[j].data(), tests[0].enc_message.size()) == 0;
            }
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_DoesNothingWhenNotInit()
{
    TestResult test_result;
    ...
    try
    {
        ...
        tc::ByteData control_block = tc::io::PaddingSource(0xdd, tc::crypto::Rsa4096OaepSha2256Encryptor::kBlockSize)
                                         .pullData(0, tc::crypto::Rsa4096OaepSha2256Encryptor::kBlockSize);
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_InitializeThrowsExceptionOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // cryptor.initialize(tc::crypto::RsaPrivateKey(tests[0].key_modulus.data(), tests[0].key_modulus.size(),
        // tests[0].key_private_exponent.data(), tests[0].key_private_exponent.size()), tests[0].label.data(),
        // tests[0].label.size(), false);

        auto label = tc::io::PaddingSource(0xab, tc::crypto::Sha2256Generator::kHashSize)
                         .pullData(0, tc::crypto::Sha2256Generator::kHashSize - 2);
        auto empty_label = tc::ByteData();

        auto key = tc::crypto::RsaPrivateKey(
            tests[0].key_modulus.data(), tests[0].key_modulus.size(), tests[0].key_private_exponent.data(),
            tests[0].key_private_exponent.size());
        auto empty_key = tc::crypto::RsaKey();
        auto bad_modulus_size_key = tc::crypto::RsaPrivateKey(
            tests[0].key_modulus.data(), tests[0].key_modulus.size() - 2, tests[0].key_private_exponent.data(),
            tests[0].key_private_exponent.size());
        auto no_modulus_key = key;
        no_modulus_key.n = tc::ByteData();
        auto bad_privexp_size_key = tc::crypto::RsaPrivateKey(
            tests[0].key_modulus.data(), tests[0].key_modulus.size(), tests[0].key_private_exponent.data(),
            tests[0].key_private_exponent.size() - 2);
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentNullException where RsaKey had a bad private exponent size");
        }
        catch (const tc::ArgumentNullException &)
        { /* do nothing */
        }
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentNullException where RsaKey had a bad public exponent size");
        }
        catch (const tc::ArgumentNullException &)
        { /* do nothing */
        }
        ...
        try
        {
            ...
            throw tc::TestException(
                "Failed to throw ArgumentNullException where RsaKey had a neither public nor private exponents");
        }
        catch (const tc::ArgumentNullException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Failed to throw correct exception where RsaKey had a neither public nor private exponents");
        }

        try
        {
            cryptor.initialize(key, label.data(), label.size(), true);
            throw tc::TestException("Failed to throw ArgumentOutOfRangeException where isLabelDigested==true but the "
                                    "label isn't the correct size");
        }
        ...
        {
            throw tc::TestException(
                "Failed to throw correct exception where isLabelDigested==true but the label isn't the correct size");
        }
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_EncryptReturnsFalseOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        // cryptor.encrypt(data.data(), tests[0].dec_message.data(), tests[0].dec_message.size(),
        // tests[0].enc_seed.data(), tests[0].enc_seed.size());

        bool result = false;

        result = cryptor.encrypt(
            nullptr, tests[0].dec_message.data(), tests[0].dec_message.size(), tests[0].enc_seed.data(),
            tests[0].enc_seed.size());
        if (result != false)
        {
        ...
        result = cryptor.encrypt(
            data.data(), nullptr, tests[0].dec_message.size(), tests[0].enc_seed.data(), tests[0].enc_seed.size());
        if (result != false)
        {
        ...
        result = cryptor.encrypt(
            data.data(), tests[0].dec_message.data(), 0, tests[0].enc_seed.data(), tests[0].enc_seed.size());
        if (result != false)
        {
            throw tc::TestException(
                "encrypt() did not return false where message_size==0"); // ArgumentOutOfRangeException
        }

        size_t max_message_size =
            tc::crypto::Rsa4096OaepSha2256Encryptor::kBlockSize - (2 * tc::crypto::Sha2256Generator::kHashSize) - 2;
        result = cryptor.encrypt(
            data.data(), tests[0].dec_message.data(), max_message_size + 1, tests[0].enc_seed.data(),
            tests[0].enc_seed.size());
        if (result != false)
        {
            throw tc::TestException("encrypt() did not return false where message_size was too large to be encrypted "
                                    "in one RSA-OAEP block"); // ArgumentOutOfRangeException
        }

        result = cryptor.encrypt(
            data.data(), tests[0].dec_message.data(), tests[0].dec_message.size(), tests[0].enc_seed.data(), 0);
        if (result != false)
        {
            throw tc::TestException(
                "encrypt() did not return false where seed_size!=HashCalculator::kHashSize"); // ArgumentOutOfRangeException
        }

        result = cryptor.encrypt(
            data.data(), tests[0].dec_message.data(), tests[0].dec_message.size(), nullptr, tests[0].enc_seed.size());
        if (result != false)
        {
...
void crypto_Rsa4096OaepSha2256Encryptor_TestClass::test_DecryptReturnsFalseOnBadInput()
{
    TestResult test_result;
    ...
    try
    {
        ...
        cryptor.initialize(
            tc::crypto::RsaPrivateKey(
                tests[0].key_modulus.data(), tests[0].key_modulus.size(), tests[0].key_private_exponent.data(),
                tests[0].key_private_exponent.size()),
            tests[0].label.data(), tests[0].label.size(), false);
        ...
        {
            throw tc::TestException(
                "decrypt() did not return false where message_capacity==0"); // ArgumentOutOfRangeException
        }
        ...
        result =
            cryptor.decrypt(data.data(), message_size, tests[0].dec_message.size() - 1, tests[0].enc_message.data());
        if (result != false)
        {
            throw tc::TestException(
                "decrypt() did not return false where message_capacity was not large enough"); // ArgumentOutOfRangeException
        }
