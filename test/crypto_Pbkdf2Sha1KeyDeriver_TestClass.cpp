...
crypto_Pbkdf2Sha1KeyDeriver_TestClass::crypto_Pbkdf2Sha1KeyDeriver_TestClass()
    : mTestTag("tc::crypto::Pbkdf2Sha1KeyDeriver"), mTestResults()
{
}
...
void crypto_Pbkdf2Sha1KeyDeriver_TestClass::test_Constants()
{
    ...
    try
    {
        ...
        static const uint64_t kExpectedMaxDerivableSize =
            uint64_t(0xffffffff) * uint64_t(tc::crypto::Sha1Generator::kHashSize);
        if (tc::crypto::Pbkdf2Sha1KeyDeriver::kMaxDerivableSize != kExpectedMaxDerivableSize)
        {
            throw tc::TestException(fmt::format(
                ...
                tc::crypto::Pbkdf2Sha1KeyDeriver::kMaxDerivableSize, kExpectedMaxDerivableSize));
        }
...
void crypto_Pbkdf2Sha1KeyDeriver_TestClass::test_WillThrowExceptionOnZeroRounds()
{
    ...
    try
    {
        ...
        try
        {
            ...
            throw tc::TestException("DeriveKeyPbkdf2Sha1() Did not throw tc::crypto::CryptoException where the round "
        ...
        catch (const tc::Exception &)
        {
            throw tc::TestException(
                "DeriveKeyPbkdf2Sha1() Threw the wrong exception where the round count was set to zero.");
        }
        ...
        try
        {
            ...
            throw tc::TestException(
                "Pbkdf2Sha1KeyDeriver::initialize() Did not throw exception where the round count was set to zero.");
        }
        catch (const tc::crypto::CryptoException &)
        { /* do nothing */
        }
        ...
        {
            throw tc::TestException(
                "Pbkdf2Sha1KeyDeriver::initialize() Threw the wrong exception where the round count was set to zero.");
        }
...
void crypto_Pbkdf2Sha1KeyDeriver_TestClass::test_WillThrowExceptionOnTooLargeDkSize()
{
    ...
    try
    {
        ...
        try
        {
            ...
            tc::crypto::DeriveKeyPbkdf2Sha1(
                nullptr, tc::crypto::Pbkdf2Sha1KeyDeriver::kMaxDerivableSize + 1,
                (const byte_t *)tests[0].in_password.c_str(), tests[0].in_password.size(),
                (const byte_t *)tests[0].in_salt.c_str(), tests[0].in_salt.size(), tests[0].in_rounds);

            throw tc::TestException("DeriveKeyPbkdf2Sha1() Did not throw tc::crypto::CryptoException where the derived "
        ...
        {
            throw tc::TestException("DeriveKeyPbkdf2Sha1() Threw the wrong exception where the derived data length "
        ...
        try
        {
            ...
            throw tc::TestException("Pbkdf2Sha1KeyDeriver::getBytes() Did not throw tc::crypto::CryptoException where "
        ...
        {
            throw tc::TestException("Pbkdf2Sha1KeyDeriver::getBytes() Threw the wrong exception where the derived data "
