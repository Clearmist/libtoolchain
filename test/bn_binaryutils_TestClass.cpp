...
bn_binaryutils_TestClass::bn_binaryutils_TestClass() : mTestTag("tc::bn (BinaryUtils)"), mTestResults() {}
...
void bn_binaryutils_TestClass::util_RoundUpFuncTestCase(uint32_t value, uint32_t alignment, uint32_t expected_result)
{
    ...
    if (result != expected_result)
    {
        throw tc::TestException(fmt::format(
            "align(0x{:x}, 0x{:x}) returned: 0x{:x} (expected: 0x{:x})", value, alignment, result, expected_result));
    }
}
...
void bn_binaryutils_TestClass::test_MakeStructMagicU32Func()
{
    ...
    try
    {
        ...
        util_MakeStructMagicU32FuncTestCase(
            "\x7F"
            "ELF",
            0x464C457F);
...
void bn_binaryutils_TestClass::util_MakeStructMagicU32FuncTestCase(
    const char *struct_magic_str, uint32_t expected_result)
{
    uint32_t result = tc::bn::make_struct_magic_uint32(struct_magic_str);
    if (result != expected_result)
    {
        throw tc::TestException(
            fmt::format("make_struct_magic_uint32() returned: 0x{:x} (expected: 0x{:x})", result, expected_result));
    }
}
...
void bn_binaryutils_TestClass::util_MakeStructMagicU64FuncTestCase(
    const char *struct_magic_str, uint64_t expected_result)
{
    uint64_t result = tc::bn::make_struct_magic_uint64(struct_magic_str);
    if (result != expected_result)
    {
        throw tc::TestException(
            fmt::format("make_struct_magic_uint64() returned: 0x{:x} (expected: 0x{:x})", result, expected_result));
    }
}
