...
void tc::string::TranscodeUtil::UTF16ToUTF32(const std::u16string &src, std::u32string &dst)
{
    ...
    for (size_t i = 0; i < src.length(); i += done)
    {
        ...
        else
        {
            ...
            if (i >= src.length() - 1 || src[i + 1] < detail::kUtf16LowSurrogateStart ||
                src[i + 1] > detail::kUtf16LowSurrogateEnd)
            {
                throw tc::ArgumentException("not a UTF-16 string");
            }

            char32_t uni = ((src[i] & detail::kUtf16SurrogateMask) << detail::kUtf16SurrogateBits) |
                           (src[i + 1] & detail::kUtf16SurrogateMask) | 0x10000;
...
void tc::string::TranscodeUtil::UTF32ToUTF16(const std::u32string &src, std::u16string &dst)
{
    ...
    for (size_t i = 0; i < src.size(); i++)
    {
        ...
        else
        {
            ...
            dst.push_back(
                ((uni >> detail::kUtf16SurrogateBits) & detail::kUtf16SurrogateMask) +
                detail::kUtf16HighSurrogateStart);
