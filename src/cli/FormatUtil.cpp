...
std::string tc::cli::FormatUtil::formatBytesAsStringWithLineLimit(
    const byte_t *data,
    size_t len,
    bool upper_case,
...
std::string
tc::cli::FormatUtil::formatBytesAsString(const byte_t *data, size_t size, bool upper_case, const std::string &delimiter)
{
    // create output string
    std::string output_str;
...
std::string
tc::cli::FormatUtil::formatBytesAsString(const tc::ByteData &data, bool upper_case, const std::string &delimiter)
{
    return formatBytesAsString(data.data(), data.size(), upper_case, delimiter);
}

std::string tc::cli::FormatUtil::formatListWithLineLimit(
    const std::vector<std::string> &str_list, size_t row_len, size_t indent_len, bool print_first_indent)
{
    if (str_list.size() == 0)
    {
        return "";
...
std::string tc::cli::FormatUtil::formatBytesAsHxdHexString(
    const byte_t *data, size_t size, size_t bytes_per_row, size_t byte_group_size)
{
