...
tc::crypto::RsaPublicKey::RsaPublicKey(
    const byte_t *modulus, size_t modulus_size, const byte_t *public_exponent, size_t public_exponent_size)
{
...
tc::crypto::RsaPrivateKey::RsaPrivateKey(
    const byte_t *modulus, size_t modulus_size, const byte_t *private_exponent, size_t private_exponent_size)
    : RsaPrivateKey(
          modulus, modulus_size, kPublicExponent, sizeof(kPublicExponent), private_exponent, private_exponent_size)
{
}
...
tc::crypto::RsaPrivateKey::RsaPrivateKey(
    ...
    size_t modulus_size,
    const byte_t *public_exponent,
    size_t public_exponent_size,
    const byte_t *private_exponent,
    size_t private_exponent_size)
{
    if (modulus != nullptr && modulus_size != 0 && public_exponent != nullptr && public_exponent_size != 0 &&
        private_exponent != nullptr && private_exponent_size != 0)
    {
        this->n = tc::ByteData(modulus, modulus_size);
        this->e = tc::ByteData(public_exponent, public_exponent_size);
