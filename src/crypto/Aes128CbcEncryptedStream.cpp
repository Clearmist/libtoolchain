...
{
    return castInt64ToUint64(offset / tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CbcEncryptor::kBlockSize));
};
inline int64_t blockIndexToOffset(uint64_t block_index)
{
    return castUint64ToInt64(block_index) * tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CbcEncryptor::kBlockSize);
};

inline size_t lengthToBlockNum(int64_t length)
{
    return tc::io::IOUtil::castInt64ToSize(
        length / tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CbcEncryptor::kBlockSize));
};
inline size_t offsetInBlock(int64_t offset)
{
    return tc::io::IOUtil::castInt64ToSize(
        offset % tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CbcEncryptor::kBlockSize));
};
...
tc::crypto::Aes128CbcEncryptedStream::Aes128CbcEncryptedStream()
    : mBaseStream(), mCryptor(std::shared_ptr<tc::crypto::Aes128CbcEncryptor>(new tc::crypto::Aes128CbcEncryptor()))
{
    memset(mBaseIv.data(), 0, mBaseIv.size());
}

tc::crypto::Aes128CbcEncryptedStream::Aes128CbcEncryptedStream(
    const std::shared_ptr<tc::io::IStream> &stream, const key_t &key, const iv_t &iv)
...
void tc::crypto::Aes128CbcEncryptedStream::setLength(int64_t length)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "::setLength()", "setLength is not supported for Aes128CbcEncryptedStream");
}
