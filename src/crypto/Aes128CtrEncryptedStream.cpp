...
// inline utils
inline uint64_t castInt64ToUint64(int64_t val)
{
    return val < 0 ? 0 : uint64_t(val);
}
inline int64_t castUint64ToInt64(uint64_t val)
{
    return (int64_t)std::min<uint64_t>(val, uint64_t(std::numeric_limits<int64_t>::max()));
}

inline uint64_t offsetToBlockIndex(int64_t offset)
{
    return castInt64ToUint64(offset / tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CtrEncryptor::kBlockSize));
};
inline int64_t blockIndexToOffset(uint64_t block_index)
{
    return castUint64ToInt64(block_index) * tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CtrEncryptor::kBlockSize);
};

inline size_t lengthToBlockNum(int64_t length)
{
    return tc::io::IOUtil::castInt64ToSize(
        length / tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CtrEncryptor::kBlockSize));
};
inline size_t offsetInBlock(int64_t offset)
{
    return tc::io::IOUtil::castInt64ToSize(
        offset % tc::io::IOUtil::castSizeToInt64(tc::crypto::Aes128CtrEncryptor::kBlockSize));
};
...
tc::crypto::Aes128CtrEncryptedStream::Aes128CtrEncryptedStream()
    : mBaseStream(), mCryptor(std::shared_ptr<tc::crypto::Aes128CtrEncryptor>(new tc::crypto::Aes128CtrEncryptor()))
{
}

tc::crypto::Aes128CtrEncryptedStream::Aes128CtrEncryptedStream(
    const std::shared_ptr<tc::io::IStream> &stream, const key_t &key, const counter_t &counter)
...
size_t tc::crypto::Aes128CtrEncryptedStream::read(byte_t *ptr, size_t count)
{
    ...
    int64_t begin_aligned_offset =
        begin_read_offset - tc::io::IOUtil::castSizeToInt64(offsetInBlock(begin_read_offset));
    int64_t end_aligned_offset =
        end_read_offset - tc::io::IOUtil::castSizeToInt64(offsetInBlock(end_read_offset)) +
        tc::io::IOUtil::castSizeToInt64(offsetInBlock(end_read_offset) ? sizeof(block_t) : 0x0);
    ...
    uint64_t continuous_begin_block_index =
        (continuous_block_num == 0) ? 0
                                    : (offsetToBlockIndex(begin_aligned_offset) + (uint64_t)read_partial_begin_block);
    ...
    */
    ...
    if (block_num == 0)
    {
        tc::InvalidOperationException(
            kClassName + "::read()", "Invalid block number (0 blocks, would have returned before now if count==0)");
    }
    ...
    if (continuous_block_num > 0)
    {
        ...
        // decrypt blocks
        mCryptor->decrypt(
            ptr + data_read_count, ptr + data_read_count, continuous_block_num * sizeof(block_t),
            continuous_begin_block_index);
...
size_t tc::crypto::Aes128CtrEncryptedStream::write(const byte_t *ptr, size_t count)
{
    if (mBaseStream == nullptr)
    {
        throw tc::ObjectDisposedException(
            kClassName + "::write()", "Failed to set stream position (stream is disposed)");
    }

    throw tc::NotSupportedException(kClassName + "::write()", "write is not supported for Aes128CtrEncryptedStream");
}
...
void tc::crypto::Aes128CtrEncryptedStream::setLength(int64_t length)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "::setLength()", "setLength is not supported for Aes128CtrEncryptedStream");
}
