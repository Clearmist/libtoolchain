...
tc::io::ConcatenatedStream::ConcatenatedStream()
    : mStreamList(), mStreamListMap(), mCurrentStream(), mCanRead(false), mCanWrite(false), mCanSeek(false),
      mStreamLength(0)
{
}

tc::io::ConcatenatedStream::ConcatenatedStream(ConcatenatedStream &&other) : ConcatenatedStream()
{
    *this = std::move(other);
}
...
size_t tc::io::ConcatenatedStream::read(byte_t *ptr, size_t count)
{
    ...
    do
    {
        ...
        size_t readable_count_for_current_stream = IOUtil::getReadableCount(
            mCurrentStream.get()->range.length, mCurrentStream.get()->stream->position(), remaining_readable_count);

        if (readable_count_for_current_stream != 0)
        {
            // read data and throw exception if unexpected read count is returned
            if (readable_count_for_current_stream !=
                mCurrentStream.get()->stream->read(
                    ptr + (readable_count - remaining_readable_count), readable_count_for_current_stream))
            {
                throw tc::io::IOException(
                    kClassName + "read()", "Reading from one of the base streams returned less data than expected.");
            }
        ...
        if (remaining_readable_count != 0)
        {
            ...
            {
                throw tc::io::IOException(
                    kClassName + "read()", "More data was expected to be readable but end of stream list was reached.");
            }
            ...
            if (mCurrentStream.get()->stream->position() != 0x0)
            {
                ...
                {
                    throw tc::io::IOException(
                        kClassName + "read()", "Tried to continue reading from the next stream but the position was "
                                               "not 0, and seek was not supported.");
                }
...
size_t tc::io::ConcatenatedStream::write(const byte_t *ptr, size_t count)
{
    ...
    do
    {
        ...
        size_t writable_count_for_current_stream = IOUtil::getWritableCount(
            mCurrentStream.get()->range.length, mCurrentStream.get()->stream->position(), remaining_writable_count);

        if (writable_count_for_current_stream != 0)
        {
            // write data and throw exception if unexpected write count is returned
            if (writable_count_for_current_stream !=
                mCurrentStream.get()->stream->write(
                    ptr + (writable_count - remaining_writable_count), writable_count_for_current_stream))
            {
                throw tc::io::IOException(
                    kClassName + "write()", "Writing from one of the base streams returned less data than expected.");
            }
        ...
        if (remaining_writable_count != 0)
        {
            ...
            if (mCurrentStream.get() == mStreamList.end())
            {
                throw tc::io::IOException(
                    kClassName + "write()",
                    "More data was expected to be writable but end of stream list was reached.");
            }
            ...
            if (mCurrentStream.get()->stream->position() != 0x0)
            {
                ...
                {
                    throw tc::io::IOException(
                        kClassName + "write()", "Tried to continue writing from the next stream but the position was "
                                                "not 0, and seek was not supported.");
                }
...
int64_t tc::io::ConcatenatedStream::seek(int64_t offset, SeekOrigin origin)
{
    ...
    if (absolute_seek_pos <= 0)
    {
        ...
        {
            throw tc::io::IOException(
                kClassName + "seek()", "Failed to seek because underlying stream could not be determined.");
        }
        mCurrentStream.get()->stream->seek(0, tc::io::SeekOrigin::Begin);
    }
    ...
    else if (absolute_seek_pos < mStreamLength)
    {
        ...
        if ((absolute_seek_pos >= mCurrentStream.get()->range.offset) &&
            (absolute_seek_pos < (mCurrentStream.get()->range.offset + mCurrentStream.get()->range.length)))
        {
            mCurrentStream.get()->stream->seek(
                absolute_seek_pos - mCurrentStream.get()->range.offset, tc::io::SeekOrigin::Begin);
        }
...
void tc::io::ConcatenatedStream::setLength(int64_t length)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "setLength()", "setLength() is not supported for tc::io::ConcatenatedStream.");
}
