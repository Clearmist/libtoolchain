...
tc::io::VirtualFileSystem::VirtualFileSystem() : mCurDir(nullptr), mFsSnapshot(), mPathResolver() {}

tc::io::VirtualFileSystem::VirtualFileSystem(
    const FileSystemSnapshot &fs_snapshot, const std::shared_ptr<tc::io::IPortablePathResolver> &path_resolver)
    : VirtualFileSystem()
{
    ...
    {
        throw tc::InvalidOperationException(
            kClassName, "Failed to located root directory. (Root path was not found in Path to Index map)");
    }
    // if the dir_entry index isn't valid, throw exception
    if (root_itr->second >= mFsSnapshot.dir_entries.size())
    {
        throw tc::InvalidOperationException(
            kClassName, "Failed to located root directory. (Root directory index was invalid)");
    }
...
{
    return mCurDir == nullptr ? tc::ResourceStatus(1 << tc::RESFLAG_NOINIT)
                              : tc::ResourceStatus(1 << tc::RESFLAG_READY);
}
...
void tc::io::VirtualFileSystem::openFile(
    ...
    std::shared_ptr<tc::io::IStream> &stream)
{
    ...
    if (mode != tc::io::FileMode::Open)
    {
        throw tc::NotSupportedException(
            kClassName + "::openFile()", "This file-system is read-only, only FileMode::Open is supported.");
    }
    if (access != tc::io::FileAccess::Read)
    {
        throw tc::NotSupportedException(
            kClassName + "::openFile()", "This file-system is read-only, only FileAccess::Read is supported.");
    }
...
void tc::io::VirtualFileSystem::createDirectory(const tc::io::Path &path)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "::createDirectory()", "createDirectory is not supported for VirtualFileSystem");
}
...
void tc::io::VirtualFileSystem::createDirectoryPath(const tc::io::Path &path)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "::createDirectoryPath()", "createDirectoryPath is not supported for VirtualFileSystem");
}
...
void tc::io::VirtualFileSystem::removeDirectory(const tc::io::Path &path)
{
    ...
    throw tc::NotSupportedException(
        kClassName + "::removeDirectory()", "removeDirectory is not supported for VirtualFileSystem");
}
...
tc::io::VirtualFileSystem::FileSystemSnapshot::FileEntry *
tc::io::VirtualFileSystem::getFileFromFsSnapshot(const std::string &method_name, const tc::io::Path &resolved_path)
{
    ...
    if (file_itr == mFsSnapshot.file_entry_path_map.end())
    {
        throw tc::io::FileNotFoundException(
            ...
            fmt::format(
                "File \"{}\" does not exist. (Path to File Index map had no match)", resolved_path.to_string()));
    }
    ...
    if (file_itr->second >= mFsSnapshot.file_entries.size())
    {
        throw tc::io::FileNotFoundException(
            kClassName + method_name,
            fmt::format("File \"{}\" does not exist. (Invalid File Index)", resolved_path.to_string()));
    }
    ...
    if (mFsSnapshot.file_entries.at(file_itr->second).stream == nullptr)
    {
        throw tc::io::FileNotFoundException(
            kClassName + method_name,
            fmt::format("File \"{}\" does not exist. (File stream was null)", resolved_path.to_string()));
    }
    // if the stream has invalid properties, throw exception
    if (!(mFsSnapshot.file_entries.at(file_itr->second).stream->canRead() == true &&
          mFsSnapshot.file_entries.at(file_itr->second).stream->canWrite() == false))
    {
        throw tc::io::FileNotFoundException(
            kClassName + method_name,
            fmt::format(
                "File \"{}\" does not exist. (File stream had invalid permissions)", resolved_path.to_string()));
    }

    return &(mFsSnapshot.file_entries.at(file_itr->second));
}

tc::io::VirtualFileSystem::FileSystemSnapshot::DirEntry *
tc::io::VirtualFileSystem::getDirectoryFromFsSnapshot(const std::string &method_name, const tc::io::Path &resolved_path)
{
    ...
    {
        throw tc::io::DirectoryNotFoundException(
            kClassName + method_name, fmt::format(
                                          "Directory \"{}\" does not exist. (Path to Directory Index map had no match)",
    ...
    if (dir_itr->second >= mFsSnapshot.dir_entries.size())
    {
        throw tc::io::DirectoryNotFoundException(
            kClassName + method_name,
            fmt::format("Directory \"{}\" does not exist. (Invalid Directory Index)", resolved_path.to_string()));
    }
