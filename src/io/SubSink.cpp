...
tc::io::SubSink::SubSink() : mBaseSink(nullptr), mBaseSinkOffset(0), mSubSinkLength(0) {}

tc::io::SubSink::SubSink(const std::shared_ptr<tc::io::ISink> &sink, int64_t offset, int64_t length) : SubSink()
{
    mBaseSink = sink;
    ...
    if (offset > (base_length - length))
    {
        throw tc::ArgumentOutOfRangeException(
            kClassName, "The sub sink offset is greater than the maximum possible "
                        "offset given the base sink length and sub sink length.");
    }
