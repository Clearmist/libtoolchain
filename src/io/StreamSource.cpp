...
tc::io::StreamSource::StreamSource() : mBaseStream(nullptr) {}

tc::io::StreamSource::StreamSource(const std::shared_ptr<tc::io::IStream> &stream) : mBaseStream(stream)
{
...
tc::ByteData tc::io::StreamSource::pullData(int64_t offset, size_t count)
{
    ...
    // read from stream (note this will not be called if mBaseStream is null, as in that case read_count == 0, and this
