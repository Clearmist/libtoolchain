...
tc::io::OverlayedSource::OverlayedSource() : mBaseSource(), mOverlaySourceInfos() {}

tc::io::OverlayedSource::OverlayedSource(
    const std::shared_ptr<tc::io::ISource> &base_source,
    const std::shared_ptr<tc::io::ISource> &overlay_source,
    int64_t offset,
    int64_t length)
...
tc::io::OverlayedSource::OverlayedSource(
    const std::shared_ptr<tc::io::ISource> &base_source, const std::vector<OverlaySourceInfo> &overlay_source_infos)
{
    ...
    for (auto itr = overlay_source_infos.begin(); itr != overlay_source_infos.end(); itr++)
    {
        ...
        if (itr->offset < 0)
        {
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::OverlayedSource()", "Invalid overlay region. Overlay offset is negative.");
        }
        ...
        {
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::OverlayedSource()",
                "Invalid overlay region. Overlay offset beyond length of base_source.");
        }
        ...
        if ((itr->offset + itr->length) > mBaseSource->length())
        {
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::OverlayedSource()",
                "Invalid overlay region. Overlay region exceeds the length of base_source.");
        }
        ...
        if (itr->length > itr->overlay_source->length())
        {
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::OverlayedSource()",
                "Invalid overlay region. Overlay region exceeds the length of overlay_source.");
        }
...
void tc::io::OverlayedSource::getOverlaySourcePullableRegion(
    int64_t base_pull_offset,
    size_t base_pull_count,
    const OverlaySourceInfo &overlay_info,
    int64_t &overlay_pull_offset,
    size_t &overlay_pull_count)
{
    ...
    else
    {
        ...
        overlay_pull_count = IOUtil::getReadableCount(
            overlay_info.length, overlay_pull_offset, size_t(overlay_relative_end_offset - overlay_pull_offset));
    }
}
