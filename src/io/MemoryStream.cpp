...
tc::io::MemoryStream::MemoryStream() : MemoryStream(0) {}

tc::io::MemoryStream::MemoryStream(size_t length) : mData(), mPosition(0)
{
    setLength(length);
}

tc::io::MemoryStream::MemoryStream(const tc::ByteData &byte_data) : mData(byte_data), mPosition(0) {}

tc::io::MemoryStream::MemoryStream(tc::ByteData &&byte_data) : mData(std::move(byte_data)), mPosition(0) {}

tc::io::MemoryStream::MemoryStream(const byte_t *data, size_t len) : mData(data, len), mPosition(0) {}
...
void tc::io::MemoryStream::setLength(int64_t length)
{
    ...
    if (IOUtil::castInt64ToSize(length) > std::numeric_limits<size_t>::max())
    {
        throw tc::ArgumentOutOfRangeException(
            kClassName + "::setLength()", "Length greater than maxium possible length for MemoryStream");
    }
