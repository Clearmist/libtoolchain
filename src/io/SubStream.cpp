...
tc::io::SubStream::SubStream() : mBaseStream(), mBaseStreamOffset(0), mSubStreamLength(0), mSubStreamPosition(0) {}
    ...
    : SubStream()
{
    ...
    if (offset > (base_length - length))
    {
        throw tc::ArgumentOutOfRangeException(
            kClassName, "SubStream offset is greater than the maximum possible "
                        "offset given the base stream size and SubStream size");
    }
