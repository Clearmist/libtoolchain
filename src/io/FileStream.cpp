...
tc::io::FileStream::FileStream()
    : mCanRead(false), mCanWrite(false), mCanSeek(false), mIsAppendRestrictSeekCall(false), mFileHandle()
{
}

tc::io::FileStream::FileStream(FileStream &&other) : FileStream()
{
    *this = std::move(other);
}

tc::io::FileStream::FileStream(const tc::io::Path &path, FileMode mode, FileAccess access) : FileStream()
{
...
int64_t tc::io::FileStream::position()
{
    ...
    if (mCanSeek == false)
    {
        throw tc::NotSupportedException(
            kClassName + "::position()", "This method is not supported for streams that do not support seeking");
    }
...
int64_t tc::io::FileStream::seek(int64_t offset, SeekOrigin origin)
{
    if (mFileHandle == nullptr)
    {
        throw tc::ObjectDisposedException(
            kClassName + "::seek()", "Failed to set stream position (stream is disposed)");
    }
    ...
    if (mIsAppendRestrictSeekCall == true)
    {
        throw tc::io::IOException(
            kClassName + "::seek()", "Streams opened in Append mode are not allowed to change file position.");
    }
...
void tc::io::FileStream::setLength(int64_t length)
{
    ...
    {
        throw tc::ObjectDisposedException(
            kClassName + "::setLength()", "Failed to set stream length (stream is disposed)");
    }

    if (mCanWrite == false || mCanSeek == false)
    {
        throw tc::NotSupportedException(
            kClassName + "::setLength()", "Stream does not support both writing and seeking");
    }
...
void tc::io::FileStream::open_impl(const tc::io::Path &path, FileMode mode, FileAccess access)
{
    ...
    {
        throw tc::ArgumentException(
            kClassName + "::open()", "Stream open mode requires write access, but write access was not allowed");
    }
    ...
    if (mode == tc::io::FileMode::Append && (access_flag & (GENERIC_READ | GENERIC_WRITE)) != GENERIC_WRITE)
    {
        throw tc::ArgumentException(
            kClassName + "::open()",
            "Stream opened in Append mode can only work with Write access. ReadWrite is not permitted");
    }

    // open file
    HANDLE file_handle = CreateFileW(
        (LPCWSTR)unicode_path.c_str(), access_flag, share_mode_flag, 0, creation_flag, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    if (file_handle == INVALID_HANDLE_VALUE)
    {
        ...
        case (ERROR_PATH_NOT_FOUND):
            throw tc::io::FileNotFoundException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetLastErrorString(error));
        case (ERROR_FILE_EXISTS):
            throw tc::io::FileExistsException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetLastErrorString(error));
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::open()",
                "Failed to open file stream (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
...
int64_t tc::io::FileStream::length_impl()
{
    ...
    if (GetFileSizeEx(mFileHandle->handle, &stream_length) == false)
    {
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::length()",
                "Failed to get stream length (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
...
size_t tc::io::FileStream::read_impl(byte_t *ptr, size_t count)
{
    ...
    if (ReadFile(mFileHandle->handle, ptr, (DWORD)count, &bytes_read, NULL) == false)
    {
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::read()",
                "Failed to read from stream (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
...
size_t tc::io::FileStream::write_impl(const byte_t *ptr, size_t count)
{
    ...
    if (WriteFile(mFileHandle->handle, ptr, (DWORD)count, &bytes_written, NULL) == false)
    {
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::write()",
                "Failed to write to stream (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
...
int64_t tc::io::FileStream::seek_impl(int64_t offset, SeekOrigin origin)
{
    ...
    if (SetFilePointerEx(mFileHandle->handle, win_pos, &out, seek_flag) == false)
    {
        DWORD error = GetLastError();
        switch (error)
        {
        case (ERROR_NEGATIVE_SEEK):
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::seek()", PlatformErrorHandlingUtil::GetLastErrorString(error));
        default:
            throw tc::io::IOException(
                kClassName + "::seek()",
                "Failed to set stream position (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
...
void tc::io::FileStream::setLength_impl(int64_t length)
{
    ...
    if (SetEndOfFile(mFileHandle->handle) == false)
    {
        DWORD error = GetLastError();
        switch (error)
        {
        default:
            throw tc::io::IOException(
                kClassName + "::setLength()",
                "Failed to set end of file (" + PlatformErrorHandlingUtil::GetLastErrorString(error) + ")");
        }
    }
}
...
void tc::io::FileStream::open_impl(const tc::io::Path &path, FileMode mode, FileAccess access)
{
    ...
    if (file_handle == -1)
    {
        ...
        case (EROFS):
            throw tc::UnauthorisedAccessException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (ENAMETOOLONG):
        ...
        case (EINVAL):
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EFAULT):
            throw tc::AccessViolationException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EISDIR):
            throw tc::io::FileNotFoundException(
                kClassName + "::open()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::open()",
                "Failed to open file stream (" + PlatformErrorHandlingUtil::GetGnuErrorNumString(errno) + ")");
        }
    }
    ...
    {
        throw tc::io::IOException(
            kClassName + "::open()", "Failed to check stream properties using fstat() (" +
...
int64_t tc::io::FileStream::length_impl()
{
    ...
    {
        throw tc::io::IOException(
            kClassName + "::length()", "Failed to check stream properties using fstat() (" +
                                           PlatformErrorHandlingUtil::GetGnuErrorNumString(errno) + ")");
    }
    ...
    {
        throw tc::NotSupportedException(
            kClassName + "::length()", "length() cannot be used with device-files or pipes");
    }
...
size_t tc::io::FileStream::read_impl(byte_t *ptr, size_t count)
{
    ...
    if (read_len == -1)
    {
        ...
        case (EINVAL):
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::read()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EFAULT):
            throw tc::AccessViolationException(
                kClassName + "::read()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EISDIR):
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::read()",
                "Failed to read from stream (" + PlatformErrorHandlingUtil::GetGnuErrorNumString(errno) + ")");
        }
    }
...
size_t tc::io::FileStream::write_impl(const byte_t *ptr, size_t count)
{
    ...
    if (write_len == -1)
    {
        ...
        case (EINVAL):
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::write()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EFAULT):
            throw tc::AccessViolationException(
                kClassName + "::write()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EFBIG):
        ...
        default:
            throw tc::io::IOException(
                kClassName + "::write()",
                "Failed to write to stream (" + PlatformErrorHandlingUtil::GetGnuErrorNumString(errno) + ")");
        }
    }
...
int64_t tc::io::FileStream::seek_impl(int64_t offset, SeekOrigin origin)
{
    ...
    if (fpos == -1)
    {
        ...
        case (EINVAL):
            throw tc::ArgumentOutOfRangeException(
                kClassName + "::seek()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EOVERFLOW):
            throw tc::OverflowException(
                kClassName + "::seek()", PlatformErrorHandlingUtil::GetGnuErrorNumString(errno));
        case (EBADF):
        case (ESPIPE):
        case (ENXIO):
        default:
            throw tc::io::IOException(
                kClassName + "::seek()",
                "Failed to set stream position (" + PlatformErrorHandlingUtil::GetGnuErrorNumString(errno) + ")");
        }
    }
