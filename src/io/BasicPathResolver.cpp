...
tc::io::BasicPathResolver::BasicPathResolver() : BasicPathResolver(tc::io::Path("/"), {}) {}
...
tc::io::BasicPathResolver::BasicPathResolver(
    const tc::io::Path &current_directory_path, const std::vector<std::string> &root_names)
    : mCurrentDirPath(), mExplicitRootLabels(root_names)
{
...
tc::io::Path tc::io::BasicPathResolver::resolveCanonicalPath(const tc::io::Path &path) const
{
    ...
    // if the begining of the path exists and is a valid root label, then the input path is an absolute (but not
    // necessarily canonical) path
    if (path_itr != path.end() &&
        (std::find(mExplicitRootLabels.begin(), mExplicitRootLabels.end(), *path_itr) != mExplicitRootLabels.end() ||
         *path_itr == mCurrentDirPath.front()))
    {
    ...
    for (; path_itr != path.end(); path_itr++)
    {
        ...
        {
            // ".." is the parent directory, so if there are path elements then we remove from the back to "go to the
            // parent directory"
