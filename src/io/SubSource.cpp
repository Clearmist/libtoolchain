...
tc::io::SubSource::SubSource() : mBaseSource(nullptr), mBaseSourceOffset(0), mSubSourceLength(0) {}
    ...
    : SubSource()
{
    ...
    if (offset > (base_length - length))
    {
        throw tc::ArgumentOutOfRangeException(
            kClassName, "sub source offset is greater than the maximum possible "
                        "offset given the base source size and sub source size");
    }
