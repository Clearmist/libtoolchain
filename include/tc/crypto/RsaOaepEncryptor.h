 ...
 */
template <size_t KeyBitSize, class HashFunction> class RsaOaepEncryptor
{
  public:
     ...
     */
    RsaOaepEncryptor() : mState(State::None), mLabelDigest(HashFunction::kHashSize), mRsaImpl(), mPrbgImpl(), mPadImpl()
    {
    }
     ...
     * @param[in] isLabelDigested Boolean indicating if label data has already been digested. False is the default
     * (label is in raw form).
     *
     * @post
    ...
    void initialize(const RsaKey &key, const byte_t *label, size_t label_size, bool isLabelDigested = false)
    {
        ...
        {
            throw tc::ArgumentNullException(
                "RsaOaepEncryptor::initialize()", "key does not have minimal required key-data.");
        }

        mRsaImpl.initialize(
            KeyBitSize, key.n.data(), key.n.size(), nullptr, 0, nullptr, 0, key.d.data(), key.d.size(), key.e.data(),
            key.e.size());

        if (((label == nullptr) ^ (label_size == 0)))
        {
            throw tc::ArgumentNullException(
                "RsaOaepEncryptor::initialize()", "label was null when label_size was non-zero or vice-versa.");
        }
        ...
        if (isLabelDigested == true)
        {
            if (label_size != HashFunction::kHashSize)
                throw tc::ArgumentOutOfRangeException(
                    "RsaOaepEncryptor::initialize()", "predigested label must be the size of HashFunction::kHashSize.");
     ...
     * - The maximum size for @p message_size is <tt>RsaOaepEncryptor::kBlockSize</tt> - (2 *
     * <tt>HashFunction::kHashSize</tt>) - 2.
     *
     * @post
     ...
     * OAEP encoding uses a random seed, this overload of @ref encrypt() generates the seed internally. To manually
     * specify the seed, please use the alternative @ref encrypt() method.
     */
    ...
    bool encrypt(byte_t *block, const byte_t *message, size_t message_size, const byte_t *seed, size_t seed_size)
    {
        ...
        if (message_size > (kBlockSize - (2 * HashFunction::kBlockSize) - 2))
        {
            return false;
        }
        if (block == nullptr)
        ...
        if (mPadImpl.BuildPad(
                encoded_message.data(), encoded_message.size(), mLabelDigest.data(), mLabelDigest.size(), message,
                message_size, seed, seed_size) != detail::RsaOaepPadding<HashFunction>::Result::kSuccess)
        {
            return false;
        }
    ...
    bool decrypt(byte_t *message, size_t &message_size, size_t message_capacity, const byte_t *block)
    {
        ...
        return (
            mPadImpl.RecoverFromPad(
                message, message_capacity, message_size, mLabelDigest.data(), mLabelDigest.size(),
                decrypted_block.data(),
                decrypted_block.size()) == detail::RsaOaepPadding<HashFunction>::Result::kSuccess);
    }
