 ...
 */
template <size_t KeyBitSize, class HashFunction> class RsaPkcs1Signer
{
  public:
     ...
     */
    RsaPkcs1Signer() : mState(State::None), mRsaImpl(), mPadImpl() {}
    ...
    void initialize(const RsaKey &key)
    {
        if (key.n.size() == 0 || (key.d.size() == 0 && key.e.size() == 0))
        {
            throw tc::ArgumentNullException(
                "RsaPkcs1Signer::initialize()", "key does not have minimal required key-data.");
        }
    ...
    bool sign(byte_t *signature, const byte_t *message_digest)
    {
        ...
        if (mPadImpl.BuildPad(block.data(), block.size(), message_digest, HashFunction::kHashSize) !=
            detail::RsaPkcs1Padding<HashFunction>::Result::kSuccess)
        {
            return false;
        }
    ...
    bool verify(const byte_t *signature, const byte_t *message_digest)
    {
        ...
        return (
            mPadImpl.CheckPad(message_digest, HashFunction::kHashSize, block.data(), block.size()) ==
            detail::RsaPkcs1Padding<HashFunction>::Result::kSuccess);
    }
