 ...
 * As such this template class requires @p HashFunction to implement one of the following hash functions to be compliant
 * with RFC 8018.
 ...
 */
template <class HashFunction> class Pbkdf2KeyDeriver
{
  public:
    static const uint64_t kMaxDerivableSize =
        uint64_t(0xffffffff) * uint64_t(HashFunction::kHashSize); /**< Maximum total key data that can be derived */
