 ...
 * std::array<byte_t, tc::crypto::Sha2512Generator::kHashSize> hash;
 *
 * // initialize generator. Sha2512Generator is now in a ready state.
 * tc::crypto::Sha2512Generator impl;
 * impl.initialize();
 *
...
class Sha2512Generator
{
  public:
     ...
     * std::array<byte_t, tc::crypto::Sha2512Generator::kHashSize> hash1;
     * impl.initialize();
     * impl.update(data.data(), data.size());
     * impl.getHash(hash1.data());
     *
     * // scenario 2 (three calls to update() 0x10 bytes each, totaling 0x30 bytes inputted)
     * std::array<byte_t, tc::crypto::Sha2512Generator::kHashSize> hash2;
     * impl.initialize();
     ...
     * std::array<byte_t, tc::crypto::Sha2512Generator::kHashSize> hash3;
     * impl.initialize();
     * impl.update(data.data() + 0x00, 0x10);
     ...
     *
     * @note
     * - If input data is broken up into blocks and supplied via multiple @ref update() calls, the order must be
 ...
 *
 * @pre
 * - The size of the <tt><var>hash</var></tt> buffer must be >= @ref Sha2512Generator::kHashSize.
 *
 * @post
 * - The calculated hash value is written to <tt><var>hash</var></tt>.
 *
 ...
 * To calculate the hash value for input split across multiple arrays, use the @ref Sha2512Generator class.
 */
void GenerateSha2512Hash(byte_t *hash, const byte_t *data, size_t data_size);
