 ...
 *
 * @details
 * Cipher block chaining (CBC) mode is intended to encrypt whole sets of data. Random access is not a feature of CBC
 * mode.
 *
 ...
 */
template <class BlockCipher> class CbcEncryptor
{
  public:
     ...
     *
     * @details
     * This updates the CBC state, initializing the initialization vector. The intended use is when data is
     * encrypted/decrypted out of order, so the initialization vector needs to be updated manually.
     *
     * @throw tc::ArgumentNullException @p key was null.
     * @throw tc::ArgumentOutOfRangeException @p key_size did not equal kKeySize.
     * @throw tc::ArgumentNullException @p iv was null.
