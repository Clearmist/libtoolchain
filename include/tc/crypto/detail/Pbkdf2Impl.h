 ...
 * -# SHA2-512
 */
template <typename HashFunction> class Pbkdf2Impl
{
  public:
    static const uint64_t kMaxDerivableSize =
        uint64_t(0xffffffff) * uint64_t(HashFunction::kHashSize); /**< Maximum total data that can be derived */

    Pbkdf2Impl()
        : mState(State::None), mPassword(), mSalt(), mRoundCount(0), mHmac(), mAvailableData(0), mTotalDataDerived(0),
          mBlockIndex(0)
    {
    ...
    void initialize(const byte_t *password, size_t password_size, const byte_t *salt, size_t salt_size, size_t n_rounds)
    {
        if (n_rounds < 1)
        {
            throw tc::crypto::CryptoException("tc::crypto::detail::Pbkdf2Impl", "Round count must be >= 1.");
        }
    ...
    void getBytes(byte_t *key, size_t key_size)
    {
        ...
        if (key_size > derivable_data)
        {
            throw tc::crypto::CryptoException("tc::crypto::detail::Pbkdf1Impl", "Request too large.");
        }
        ...
        while (key_size != 0)
        {
            ...
            size_t copy_size = std::min<size_t>(
                key_size, size_t(std::min<uint64_t>(mAvailableData, std::numeric_limits<size_t>::max())));
            ...
            // decrement available digest so the next loop can determine where to copy from and generate more digest if
            // needed
