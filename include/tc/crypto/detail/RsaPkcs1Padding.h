 ...
 */
template <typename HashFunction> class RsaPkcs1Padding
{
  public:
    ...
    RsaPkcs1Padding::Result
    BuildPad(byte_t *out_block, size_t block_size, const byte_t *message_digest, size_t message_digest_size)
    {
        ...
        if (block_size < 2 + 1 + HashFunction::kAsn1OidData.size() + kHashSize)
        {
    ...
    RsaPkcs1Padding::Result
    CheckPad(const byte_t *message_digest, size_t message_digest_size, byte_t *block, size_t block_size)
    {
        if (message_digest_size != kHashSize)
        {
            return Result::kBadMessageDigestSize;
