 ...
 */
template <class BlockCipher> class CmacImpl
{
  public:
    static_assert(
        BlockCipher::kBlockSize == 8 || BlockCipher::kBlockSize == 16,
        "CmacImpl only supports BlockCiphers with block size 8 or 16.");
    ...
    CmacImpl() : mBlockSize(kBlockSize), mState(State::None), mCipher() {}

    void initialize(const byte_t *key, size_t key_size)
    {
        if (key == nullptr)
        {
            throw tc::ArgumentNullException("CmacImpl::initialize()", "key was null.");
        }
        if (key_size != kKeySize)
        {
            throw tc::ArgumentOutOfRangeException("CmacImpl::initialize()", "key_size did not equal kKeySize.");
        }
    ...
    void update(const byte_t *data, size_t data_size)
    {
        ...
        if (mState == State::Initialized)
        {
            ...
            // where update was called and there is data in mUnprocessedBlock (from previous call to update()), this
            // must be handled first before processing new data
            if (mUnprocessedBlockPos > 0)
            {
                // try to complete block (with data if required)
                size_t remaining_block_len =
                    mUnprocessedBlock.size() - std::min<size_t>(mUnprocessedBlock.size(), mUnprocessedBlockPos);
                ...
                // if unprocessed block is now complete, and there is more data to process -> update MAC and clear
                // unprocessed block otherwise leave unprocessed block to be processed later
            ...
            if (block_num > 0)
            {
                ...
                // mUnprocessedBlockPos will always be zero/empty by this point as it must be cleared during the start
                // of update
  ...
  private:
    ...
    inline void
    derive_cmac_keys(BlockCipher &cipher, std::array<byte_t, kBlockSize> &K1, std::array<byte_t, kBlockSize> &K2)
    {
