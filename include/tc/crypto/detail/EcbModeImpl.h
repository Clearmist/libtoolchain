 ...
 */
template <class BlockCipher> class EcbModeImpl
{
  public:
    static const size_t kKeySize = BlockCipher::kKeySize;
    static const size_t kBlockSize = BlockCipher::kBlockSize;

    EcbModeImpl() : mState(None), mCipher() {}

    void initialize(const byte_t *key, size_t key_size)
    {
        if (key == nullptr)
        {
            throw tc::ArgumentNullException("EcbModeImpl::initialize()", "key was null.");
        }
        if (key_size != kKeySize)
        {
            throw tc::ArgumentOutOfRangeException("EcbModeImpl::initialize()", "key_size did not equal kKeySize.");
        }
    ...
    void encrypt(byte_t *dst, const byte_t *src, size_t size)
    {
        ...
        if (dst == nullptr)
        {
            throw tc::ArgumentNullException("EcbModeImpl::encrypt()", "dst was null.");
        }
        if (src == nullptr)
        {
            throw tc::ArgumentNullException("EcbModeImpl::encrypt()", "src was null.");
        }
        if (size < kBlockSize)
        {
            throw tc::ArgumentOutOfRangeException("EcbModeImpl::encrypt()", "size was less than kBlockSize.");
        }
    ...
    void decrypt(byte_t *dst, const byte_t *src, size_t size)
    {
        ...
        if (dst == nullptr)
        {
            throw tc::ArgumentNullException("EcbModeImpl::decrypt()", "dst was null.");
        }
        if (src == nullptr)
        {
            throw tc::ArgumentNullException("EcbModeImpl::decrypt()", "src was null.");
        }
        if (size < kBlockSize)
        {
            throw tc::ArgumentOutOfRangeException("EcbModeImpl::decrypt()", "size less than kBlockSize.");
        }
