...
/**
 * @class XtsModeImpl
 * @brief This class implements the XTS (<b>X</b>EX mode with cipher<b>t</b>ext <b>s</b>tealing) mode cipher as a
 * template class.
 *
 ...
 */
template <class BlockCipher> class XtsModeImpl
{
  public:
    ...
    XtsModeImpl() : mState(None), mCryptCipher(), mTweakCipher(), mSectorSize(0), mTweakIsLittleEndian(true) {}
    ...
    void initialize(
        ...
        size_t key2_size,
        size_t sector_size,
        bool tweak_little_endian = true)
    {
        if (key1 == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::initialize()", "key1 was null.");
        }
        if (key1_size != kKeySize)
        {
            throw tc::ArgumentOutOfRangeException("XtsModeImpl::initialize()", "key1_size did not equal kKeySize.");
        }
        if (key2 == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::initialize()", "key2 was null.");
        }
        if (key2_size != kKeySize)
        {
            throw tc::ArgumentOutOfRangeException("XtsModeImpl::initialize()", "key2_size did not equal kKeySize.");
        }
        if (sector_size < kBlockSize)
        {
            throw tc::ArgumentOutOfRangeException("XtsModeImpl::initialize()", "sector_size was less than kBlockSize.");
        }
    ...
    void encrypt(byte_t *dst, const byte_t *src, size_t size, uint64_t sector_number)
    {
        ...
        if (dst == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::encrypt()", "dst was null.");
        }
        if (src == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::encrypt()", "src was null.");
        }
        if (size == 0 || size % mSectorSize)
        {
            throw tc::ArgumentOutOfRangeException(
                "XtsModeImpl::encrypt()", "size was not a multiple of the sector size.");
        }
        ...
        for (size_t sector_idx = 0, sector_num = (size / mSectorSize); sector_idx < sector_num; sector_idx++)
        {
            ...
            if (sector_leftover > 0)
            {
                ...
                {
                    // block [sector_leftover, kBlockSize) = prev_dst_block[sector_leftover, kBlockSize) ^
                    // enc_tweak[sector_leftover, kBlockSize)
    ...
    void decrypt(byte_t *dst, const byte_t *src, size_t size, uint64_t sector_number)
    {
        ...
        if (dst == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::decrypt()", "dst was null.");
        }
        if (src == nullptr)
        {
            throw tc::ArgumentNullException("XtsModeImpl::decrypt()", "src was null.");
        }
        if (size == 0 || size % mSectorSize)
        {
            throw tc::ArgumentOutOfRangeException("XtsModeImpl::decrypt()", "size was not a multiple of sector_size.");
        }
        ...
        for (size_t sector_idx = 0, sector_num = (size / mSectorSize); sector_idx < sector_num; sector_idx++)
        {
            ...
            for (size_t block_idx = 0, block_num = (mSectorSize / kBlockSize); block_idx < block_num; block_idx++)
            {
                ...
                if ((block_idx + 1) == block_num && sector_leftover > 0)
                {
                    ...
                    // Update encrypted tweak since this block uses the next tweak due to encryption mode cipher text
                    // stealing
                    galois_func(enc_tweak.data());
                }
            ...
            if (sector_leftover > 0)
            {
                ...
                {
                    // block [sector_leftover, kBlockSize) = prev_dst_block[sector_leftover, kBlockSize) ^
                    // prev_tweak[sector_leftover, kBlockSize)
