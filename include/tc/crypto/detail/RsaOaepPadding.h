 ...
 */
template <typename HashFunction> class RsaOaepPadding
{
  public:
    ...
    RsaOaepPadding::Result BuildPad(
        byte_t *out_block,
        size_t block_size,
        const byte_t *label_digest,
        size_t label_digest_size,
        const byte_t *raw_message,
        size_t raw_message_size,
        const byte_t *seed,
        size_t seed_size)
    {
        if (seed_size != kHashSize)
        {
            return Result::kBadSeedSize;
        }
        if (label_digest_size != kHashSize)
        {
            return Result::kBadLabelDigestSize;
        }
        if (block_size < (1 + seed_size + label_digest_size + 1 + raw_message_size))
        {
        ...
        // apply mask
        apply_mgf1_mask<kHashSize>(
            out_block + label_digest_offset, block_size - label_digest_offset, out_block + seed_offset, seed_size);
        apply_mgf1_mask<kHashSize>(
            out_block + seed_offset, seed_size, out_block + label_digest_offset, block_size - label_digest_offset);

        return Result::kSuccess;
    }

    RsaOaepPadding::Result RecoverFromPad(
        byte_t *out_message,
        size_t out_size,
        size_t &message_size,
        const byte_t *label_digest,
        ...
        size_t block_size)
    {
        ...
        if (block_size < (1 + seed_size + label_digest_size + 1 + 1))
        {
            return Result::kBlockSizeTooSmall;
        }
        ...
        // apply mask
        apply_mgf1_mask<kHashSize>(
            block + seed_offset, seed_size, block + label_digest_offset, block_size - label_digest_offset);
        apply_mgf1_mask<kHashSize>(
            block + label_digest_offset, block_size - label_digest_offset, block + seed_offset, seed_size);
