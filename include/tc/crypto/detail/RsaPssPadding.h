 ...
 */
template <typename HashFunction> class RsaPssPadding
{
  public:
     ...
     */
    RsaPssPadding::Result BuildPad(
        byte_t *out_block,
        size_t block_size,
        const byte_t *message_digest,
        size_t message_digest_size,
        const byte_t *salt,
        size_t salt_size,
        size_t modulus_msb)
    {
        ...
        if (message_digest_size != kHashSize)
        {
            return Result::kBadMessageDigestSize;
        }

        // salt_size cannot have any variance from the expected size
        if (salt_size != expected_salt_size)
        {
            return Result::kBadSaltSize;
        }
     ...
     * @note Where (modulus_msb % 8 == 0) this fails tests. Investigation required.
     */
    RsaPssPadding::Result CheckPad(
        const byte_t *message_digest, size_t message_digest_size, byte_t *block, size_t block_size, size_t modulus_msb)
    {
        size_t min_salt_size = kHashSize - 2;
        size_t salt_size = 0;
         ...
         */
        if (block[0] >> (8 - block_size * 8 + modulus_msb))
        {
            return Result::kBadInputData;
        }
  ...
  private:
    ...
    inline void
    compute_encoded_message_digest(byte_t *dst, const byte_t *message_digest, const byte_t *salt, size_t salt_size)
    {
        HashFunction hash;
