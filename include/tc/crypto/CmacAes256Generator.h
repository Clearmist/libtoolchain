/**
 * @file CmacAes256Generator.h
 * @brief Declarations for API resources for CMAC-AES-256 calculations.
 * @author Jack (jakcron)
 * @version 0.1
 * @date 2024/12/10
 **/
#pragma once
#include <tc/crypto/AesEncryptor.h>
#include <tc/crypto/CmacGenerator.h>
#include <tc/types.h>

namespace tc
{
namespace crypto
{

/**
 * @typedef CmacAes256Generator
 * @brief Class for calculating CMAC-AES-256.
 *
 * @details This class calcualtes CMAC using AES-256.
 * For more information refer to @ref CmacGenerator.
 */
using CmacAes256Generator = CmacGenerator<Aes256Encryptor>;

/**
 * @brief Utility function for calculating CMAC-AES-256.
 *
 * @param[out] mac Pointer to the buffer storing the MAC.
 * @param[in]  data Pointer to input data.
 * @param[in]  data_size Size in bytes of input data.
 * @param[in]  key Pointer to key data.
 * @param[in]  key_size Size in bytes of key data.
 *
 * @pre
 * - Size of the MAC buffer must >= <tt>CmacAes256Generator::kMacSize</tt>.
 * - @p key_size == <tt>CmacAes256Generator::kMacSize</tt>.
 *
 * @post
 * - The MAC is written to <tt><var>mac</var></tt>.
 *
 * @details
 * This function calculates a MAC for the passed in data array.
 * To calculate a MAC for data split into multiple arrays, use the @ref CmacAes256Generator class.
 */
void GenerateCmacAes256Mac(byte_t *mac, const byte_t *data, size_t data_size, const byte_t *key, size_t key_size);

} // namespace crypto
} // namespace tc
