 ...
 */
template <size_t KeyBitSize, class HashFunction> class RsaPssSigner
{
  public:
     ...
     */
    RsaPssSigner() : mState(State::None), mRsaImpl(), mPrbgImpl(), mPadImpl() {}
    ...
    void initialize(const RsaKey &key)
    {
        if (key.n.size() == 0 || (key.d.size() == 0 && key.e.size() == 0))
        {
            throw tc::ArgumentNullException(
                "RsaPssSigner::initialize()", "key does not have minimal required key-data.");
        }
    ...
    bool sign(byte_t *signature, const byte_t *message_digest)
    {
        ...
        // usable_salt_size: salt.size(), but if the blocksize isn't big enough, then its = KeySize -
        // (HashFunction::kHashSize + 2) this may produce an illegal salt size (salt_size < HashFunction::kHashSize -
        // 2), but this will be caught by BuildPad()
     ...
     * - Where KeySize >= (HashCalculator::kHashSize * 2 + 2) @p salt_size = HashCalculator::kHashSize. Otherwise @p
     * salt_size = KeySize - (HashCalculator::kHashSize + 2). However the minimum legal salt size is
     * (HashCalculator::kHashSize - 2), if the salt_size falls below this consider a larger KeySize as this operation
    ...
    bool sign(byte_t *signature, const byte_t *message_digest, const byte_t *salt, size_t salt_size)
    {
        ...
        if (mPadImpl.BuildPad(
                block.data(), block.size(), message_digest, HashFunction::kHashSize, salt, salt_size, KeyBitSize - 1) !=
            detail::RsaPssPadding<HashFunction>::Result::kSuccess)
        {
            return false;
        }
    ...
    bool verify(const byte_t *signature, const byte_t *message_digest)
    {
        ...
        return (
            mPadImpl.CheckPad(
                message_digest, HashFunction::kHashSize, block.data(), block.size(), kSignatureSize - 1) ==
            detail::RsaPssPadding<HashFunction>::Result::kSuccess);
    }
