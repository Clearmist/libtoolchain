 ...
 * @brief The exception that is thrown when a null reference is passed to a method that does not accept it as a valid
 *argument.
 **/
class ArgumentNullException : public tc::ArgumentException
{
  public:
    /// Default Constructor
    ArgumentNullException() noexcept : tc::ArgumentException() {}
     ...
     * - error() == what
     **/
    ArgumentNullException(const std::string &what) noexcept : tc::ArgumentException(what) {}
