 ...
 *
 * This class processes command-line options according to user defined implementations of @ref
 * tc::cli::OptionParser::IOptionHandler that are registered with this class.
 *
 * The format of command-line arguments varies by system and convention. This class supports the following styles of
 ...
 * * @b -opt @b var1 @b var2 : Option name prefixed by "-" with one or more parameters delimited by white space,
 *
 * When parsing options from command-line arguments, it will (in order of occurence) collect the option name and the
 * parameters that follow in accordance to the above rules, and defer to the user defined @ref
 * tc::cli::OptionParser::IOptionHandler for this option. If none is defined it will throw a @ref tc::ArgumentException,
 * or alternatively defer to a user defined unknown option handler (also a @ref tc::cli::OptionParser::IOptionHandler).
 *
 ...
 * @code
 * someprogram -sku <your SKU code here> -DVAR1=<value for variable key VAR1 here> -DVAR2=<value for variable key VAR2
 * here>
 * @endcode
 *
 ...
 * 	{}
 *
 * 	// OptionParser uses this to determine which IOptionHandler to use, so this should return all aliases of the
 * option const std::vector<std::string>& getOptionStrings() const
 * 	{
 * 		return mOptStrings;
 * 	}
 *
 * 	// OptionParser uses this to determine which IOptionHandler to use, so this should return all regex patterns
 * that will match for the option const std::vector<std::string>& getOptionRegexPatterns() const
 * 	{
 * 		return mOptRegex;
 * 	}
 *
 ...
 * 		mUserOpt(user_opt),
 * 		mOptStrings(),
 * 		mOptRegex({"(-D)(.+)"}) // here we define a REGEX pattern to match the beginning of the option "-D"
 * followed by the key.
 * 	{}
 *
 * 	// OptionParser uses this to determine which IOptionHandler to use, so this should return all aliases of the
 ...
 *
 * 	// since args will include at args[0], the program executable path, use the overload of processOptions that
 * selects a sub vector of args. opt_parser.processOptions(args, 1, args.size()-1);
 *
 * 	// user_opt.sku_type will now be populated if it was set via command-line with "-sku"
 * 	// user_opt.environment_vars will now be populated if it was set via command-line with "-DKEY=VALUE" style
 * options.
 *
  ...
  public:
    ...
    class IOptionHandler
    {
      public:
         ...
         * @param[in] params This is a vector of parameters that were supplied with the option name, size may be 0 or
         * more.
         */
    ...
    /**
     * @brief Register an IOptionHandler to handle all unrecognised options. Only use this extra processing of
     * unrecognised options is required beyond a generic exception.
     *
     * @param[in] handler Shared pointer to the IOptionHandler.
     *
     * @throw tc::ArgumentNullException @p handler was null.
