 ...
 *	{
 *		uint32_t version;
 *		tc::bn::string<16> product_sku; // this has a ENCODED_SIZE & LOGICAL_SIZE of 16 bytes which mean's the
 *size on disk is 16 bytes and there are 16 usable characters tc::bn::string<32,31> product_title; // this has a
 *ENCODED_SIZE of 32 bytes & LOGICAL_SIZE of 31 bytes which mean's the size on disk is 32 bytes and there are 31 usable
 *characters uint32_t product_version;
 *	};
 * @endcode
 * In the above struct the correct size of @c product_sku and @c product_title are preserved while also enforcing the
 *logical size of the string.
 *
 ...
 * @endcode
 */
template <size_t ENCODED_SIZE, size_t LOGICAL_SIZE = ENCODED_SIZE> class string
{
  public:
    ...
    void encode(const std::string &source_str)
    {
        ...
        for (; chr_count < LOGICAL_SIZE; chr_count++)
        {
            ...
            if (chr_count >= source_str.size())
                break;
            if (source_str[chr_count] == 0)
                break;
