 ...
 * @brief This implementation of IPortablePathResolver resolves a path and current directory to canonical path,
 * resolving only '`.`', '`..`' and empty path elements.
 * @details
 * This does not consider the local file-system/environment, so links or '`~`' will not be resolved properly. It is
 * intended for processing archived/portable filesystems.
 *
 ...
 *
 * BasicPathResolver maintains a list of root labels to determine if a path being resolved is absolute or relative,
 * including:
 * * Implicit Root Label : This is the first element in the current working directory, will change if the current
...
class BasicPathResolver : public tc::io::IPortablePathResolver
{
  public:
     ...
     *
     * @post The current directory will be @p current_directory_path and the list of explicit root labels will be @p
     * root_labels .
     */
