 ...
 *
 * @details
 * The intended use-case for VirtualFileSystem is for when the user must read/parse the file-system structures directly
 *instead of using the OS to mount it. The user generates a file-system snapshot from the file-system structures (see
 *@ref VirtualFileSystem::FileSystemSnapshot) and supplies that to VirtualFileSystem to create a IFileSystem shim.
 *
 * User supplies:
 * * a @ref VirtualFileSystem::FileSystemSnapshot struct which contains vectors of directory and file entries, including
 *mapping between absolute tc::io::Path and a dir/file entry.
 * * optionally an implementation of @ref tc::io::IPortablePathResolver to determine the absolute path from a relative
 *path and the current directory. Providing a custom IPortablePathResolver implementation is only required when special
 *logic (like case insensitivity) is required to resolve the correct absolute path.
 **/
class VirtualFileSystem : public tc::io::IFileSystem
{
  public:
     ...
     * @brief A struct which contains vectors of directory and file entries, including mapping between absolute
     * tc::io::Path and a dir/file entry.
     *
     * @details
     * FileSystemSnapshot is snapshot of a FileSystem directory tree. It only has to support read-only access to
     * directory information and file streams.
     *
    ...
    struct FileSystemSnapshot
    {
        FileSystemSnapshot() : dir_entries(), file_entries(), dir_entry_path_map(), file_entry_path_map() {}
         ...
         *
         * DirEntries are used to confirm that a directory exists, and also to provide the sDirectoryListing for
         * getDirectoryListing()
         */
        struct DirEntry
        {
         ...
         * IStream objects should have .canRead() == true AND .canWrite() == false, canSeek() is preferred to be true,
     ...
     * @post This will create an unusable VirtualFileSystem, it will have to be assigned from a valid VirtualFileSystem
     *object to be usable.
     **/
    VirtualFileSystem();
     ...
     *
     * @param[in] fs_snapshot The FileSystemSnapshot object which this VirtualFileSystem will use to process file-system
     *requests.
     * @param[in] path_resolver Pointer to @ref tc::io::IPortablePathResolver object that resolves relative paths to
     *absolute paths. If @p nullptr, @ref tc::io::BasicPathResolver will be used.
     *
     * @throw tc::InvalidOperationException @p fs_snapshot Did not contain a root directory entry.
     **/
    VirtualFileSystem(
        const FileSystemSnapshot &fs_snapshot,
        const std::shared_ptr<tc::io::IPortablePathResolver> &path_resolver = nullptr);
     ...
     *
     * @param[in] path A relative or absolute path to file.
     * @param[in] mode One of the enumeration values that determines how to open or create the file. This must be @ref
     *tc::io::FileMode::Open for VirtualFileSystem.
     * @param[in] access One of the enumeration values that determines how the file can be accessed by the @ref IStream
     *object. This must be @ref tc::io::FileAccess::Read for VirtualFileSystem.
     * @param[out] stream Pointer to IStream object to be instantiated.
     *
     * @throw tc::ObjectDisposedException Methods were called after the file-system was closed.
     * @throw tc::NotSupportedException Unsupported access/mode ( @p mode was not @ref tc::io::FileMode::Open, or @p
     *access was not @ref tc::io::FileAccess::Read).
    ...
    /**
     * @brief Create a directory path. This is similar to @ref createDirectory() but it will create a directory for each
     *element in the path.
     * @details This method is not supported for VirtualFileSystem.
     *
     * @param[in] path A relative or absolute path to directory.
  ...
  private:
    ...
    FileSystemSnapshot::FileEntry *
    getFileFromFsSnapshot(const std::string &method_name, const tc::io::Path &resolved_path);
    FileSystemSnapshot::DirEntry *
    getDirectoryFromFsSnapshot(const std::string &method_name, const tc::io::Path &resolved_path);
};
