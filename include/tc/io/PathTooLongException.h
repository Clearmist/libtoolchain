 ...
 * @brief The exception that is thrown when a path or fully qualified file name is longer than the system-defined
 *maximum length.
 **/
class PathTooLongException : public tc::io::IOException
{
  public:
    /// Default Constructor
    PathTooLongException() noexcept : tc::io::IOException() {}
     ...
     * - error() == what
     **/
    PathTooLongException(const std::string &what) noexcept : tc::io::IOException(what) {}
