 ...
 * * Root Name (optional) - This is collection of reserved path elements that identifies the root or one of the roots on
 *the file system.
 ...
 * 	 * These can be real directories like "foo", or contextual directories like "." (current directory), ".."
 *(parent directory)
 ...
 * * Canonical - This is similar to an absolute path, but it is the simplest possible absolute path, containing no
 *contextual path elements
 **/
class Path
{
  public:
     ...
     * @param[in] len Number of path elements. Default value is @ref npos, indicating include all path elements after @p
     * pos.
     ...
     *
     * @param[in] format @ref tc::io::Path::Format format to encode path as string. Default is @ref
     * tc::io::Path::Format::Native.
