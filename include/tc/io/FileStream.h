...
class FileStream : public tc::io::IStream
{
  public:
     ...
     * @param[in] access One of the enumeration values that determines how the file can be accessed by the FileStream
     *object. This also determines the values returned by the @ref canRead and @ref canWrite methods of the FileStream
     *object. @ref canSeek is true if path specifies a disk file.
     *
     ...
     * @throw tc::io::IOException An I/O error, such as specifying @p mode @a CreateNew when the file specified by @p
     ...
     * @throw tc::UnauthorisedAccessException The @p access requested is not permitted by the operating system for the
     *specified @p path, such as when @p access is @a Write or @a ReadWrite and the file or directory is set for
     *read-only access.
     * @throw tc::io::PathTooLongException The specified @p path, file name, or both exceed the system-defined maximum
     *length.
    ...
    /**
     * @brief Reads a sequence of bytes from the current stream and advances the position within the stream by the
     *number of bytes read.
     *
     * @param[out] ptr Pointer to an array of bytes. When this method returns, @p ptr contains the specified byte array
     *with the values between 0 and (@p count - 1) replaced by the bytes read from the current source.
     * @param[in] count The maximum number of bytes to be read from the current stream.
     *
     * @return The total number of bytes read into @p ptr. This can be less than the number of bytes requested if that
    ...
    /**
     * @brief Writes a sequence of bytes to the current stream and advances the current position within this stream by
     ...
     *
     * @return The total number of bytes written to the stream. This can be less than the number of bytes requested if
     ...
     * @param[in] origin A value of type @ref tc::io::SeekOrigin indicating the reference point used to obtain the new
     *position.
     *
     ...
     * @throw tc::NotSupportedException The stream does not support seeking, such as if the stream is constructed from a
     ...
     * @note Use @ref canSeek to determine if this stream supports seeking.
     *
     * @throw tc::NotSupportedException The stream does not support both writing and seeking, such as if the stream is
