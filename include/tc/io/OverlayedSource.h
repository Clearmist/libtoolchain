 ...
 * @brief This will replaces regions within a base source with one or more other sources, so as to override those
 *regions in the base source when pullData() is called.
 **/
class OverlayedSource : public tc::io::ISource
{
  public:
     ...
     * @throw tc::ArgumentOutOfRangeException The overlay region offset is negative or the total size exceeded the
     *length of @p base_source .
     **/
    OverlayedSource(
     ...
     * @throw tc::ArgumentOutOfRangeException An overlay source was smaller than the region in the base source it was
     *supposed to overlay.
     * @throw tc::ArgumentOutOfRangeException A region to overlay in the base source either partly or entirely does not
     *exist.
     **/
    OverlayedSource(
        const std::shared_ptr<tc::io::ISource> &base_source,
        const std::vector<OverlaySourceInfo> &overlay_source_infos);
  ...
  private:
    ...
    void getOverlaySourcePullableRegion(
